...
# Добавляем методы в самый конец файла с минимальным синтаксисом
new_methods = """
    def _load_user_preferences(self, user_id: str):
        # Загружает пользовательские предпочтения
        preferences = {"language": "ru", "theme": "light", "notifications": {"email": True, "push": True}}
    ...
    def _create_sample_project(self, user_id: str, project_name: str = "Демо-проект"):
        ...
        try:
            import uuid
            project = {"id": str(uuid.uuid4()), "name": project_name, "url": "https://example.com", "owner_id": user_id, "status": "active"}
    ...
    def _track_onboarding_completion(self, user_id: str, data: dict):
        ...
        try:
            ...
            record = {"user_id": user_id, "completed_at": datetime.now().isoformat(), "steps_completed": len(self.progress.get("completed_steps", [])), "plan": self.plan}
...
# Просто добавляем в конец файла
...
print("✅ Методы добавлены с простым синтаксисом!")
