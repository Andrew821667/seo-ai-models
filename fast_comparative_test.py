...
"""
Быстрое сравнительное тестирование улучшенных JavaScript-компонентов.
"""
...
# Сокращенный список тестовых сайтов
TEST_SITES = [
    {
        "name": "React Router",
...
def test_site(site_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        output_file = os.path.join(results_dir, f"{name.lower().replace(' ', '_')}_fixed.json")
...
def main():
    ...
    logger.info(f"Запуск быстрого тестирования на {len(TEST_SITES)} сайтах")
    ...
    summary_file = os.path.join(results_dir, "summary_fixed.json")
    with open(summary_file, 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
        
    logger.info(f"Сводный отчет сохранен в {summary_file}")
    logger.info("Быстрое тестирование завершено")
