...
def generate_recommendations(input_file: str, output_file: str) -> bool:
    ...
    try:
        ...
        # === 0. ПРОВЕРКА НА ПУСТЫЕ ДАННЫЕ ===
        # Проверяем базовые метрики контента
        word_count = analysis_data.get('word_count', 0)
        sentence_count = analysis_data.get('sentence_count', 0)
        headers_count = analysis_data.get('headers_count', 0)
        paragraphs_count = analysis_data.get('paragraphs_count', 0)

        # Если все основные метрики = 0, значит контент не был получен
        if word_count == 0 and sentence_count == 0 and headers_count == 0:
            basic_recommendations['critical'] = [
                "🔴 КРИТИЧЕСКАЯ ОШИБКА: Анализатор не смог получить контент со страницы!",
                "",
                "**Возможные причины:**",
                "1. Сайт использует JavaScript для рендеринга контента (SPA)",
                "2. Сайт блокирует автоматический доступ/ботов",
                "3. Требуется авторизация для просмотра контента",
                "4. Проблемы с подключением или таймауты",
                "5. Страница защищена Cloudflare или подобными сервисами",
                "",
                "**Рекомендации:**",
                "- Проверьте доступность сайта в браузере",
                "- Убедитесь, что Playwright установлен и работает корректно",
                "- Проверьте логи GitHub Actions на наличие ошибок парсинга",
                "- Увеличьте таймауты для загрузки страницы",
                "- Рассмотрите использование альтернативных методов парсинга"
            ]

            # Не анализируем дальше, если нет базовых данных
            print("⚠️ ВНИМАНИЕ: Обнаружены нулевые метрики контента!")
        ...
        # Если нет рекомендаций (и нет критических ошибок)
        if not basic_recommendations:
            basic_recommendations['general'] = [
                '✅ Основные SEO-метрики в хорошем состоянии!'
            ]
        elif len(basic_recommendations) == 1 and 'critical' in basic_recommendations:
            # Если есть только критические ошибки, добавим примечание
            basic_recommendations['note'] = [
                "⚠️ Дальнейший анализ невозможен без получения контента страницы"
            ]
