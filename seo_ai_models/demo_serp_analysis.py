...
from seo_ai_models.models.llm_integration.serp_analysis.competitor_tracking import (
    CompetitorTracking,
)
from seo_ai_models.models.llm_integration.serp_analysis.industry_benchmarker import (
    IndustryBenchmarker,
)
...
def demonstrate_serp_analyzer(services: Dict[str, Any], query: str, content: str) -> None:
    ...
    if len(content) > 500:
        ...
        logger.info(
            f"Лучший вариант: #{comparison_result.get('best_variant', {}).get('index', 0) + 1}"
        )
        logger.info(
            f"Видимость лучшего варианта: {comparison_result.get('best_variant', {}).get('visibility_score', 0):.2f}"
        )
        logger.info(
            f"Оценки видимости вариантов: {[f'{score:.2f}' for score in comparison_result.get('visibility_scores', [])]}"
        )


def demonstrate_citation_analyzer(
    services: Dict[str, Any], content: str, queries: List[str]
) -> None:
    """
    ...
    result = citation_analyzer.analyze_citation(
        content=content, queries=queries, llm_engines=["perplexity"], num_samples=1, budget=50.0
    )
    ...
    logger.info(
        f"Корреляция плотности ключевых слов: {correlation_info.get('keyword_density_correlation', 0):.2f}"
    )


def demonstrate_competitor_tracking(
    services: Dict[str, Any], query: str, our_content: str, competitors: List[Dict[str, Any]]
) -> None:
    """
...
def demonstrate_industry_benchmarker(
    services: Dict[str, Any], content: str, queries: List[str], industry: str, content_type: str
) -> None:
    """
    ...
    logger.info(
        f"Минимальная длина контента: {content_metrics.get('min_content_length', 0)} символов"
    )
    logger.info(
        f"Оптимальная длина контента: {content_metrics.get('optimal_content_length', 0)} символов"
    )
    logger.info(
        f"Оптимальное количество заголовков: {content_metrics.get('optimal_headers_count', 0)}"
    )
    ...
    # Выводим основные результаты
    logger.info(
        f"Частота цитирования: {result.get('citation_rate', 0):.2%} (vs бенчмарк: {result.get('citation_rate_vs_benchmark', 0):.2f})"
    )
    logger.info(
        f"Видимость: {result.get('visibility_score', 0):.2f} (vs бенчмарк: {result.get('visibility_vs_benchmark', 0):.2f})"
    )
...
def get_sample_data() -> Dict[str, Any]:
    ...
    return {
        "our_content": our_content,
        "competitors": [
            {"id": "competitor1", "name": "SEO Expert Blog", "content": competitor1_content},
            {
                "id": "competitor2",
...
def main():
    ...
    parser.add_argument(
        "--demo",
        choices=["all", "serp", "citation", "competitors", "benchmarks"],
        default="all",
    ...
    if args.demo in ["all", "competitors"]:
        demonstrate_competitor_tracking(
            services, sample_data["query"], sample_data["our_content"], sample_data["competitors"]
        )

    if args.demo in ["all", "benchmarks"]:
        demonstrate_industry_benchmarker(
            services,
            sample_data["our_content"],
            sample_data["queries"],
            sample_data["industry"],
            sample_data["content_type"],
        )
