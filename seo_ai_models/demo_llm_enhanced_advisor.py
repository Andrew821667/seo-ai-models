...
def analyze_content(
    api_key: str, content: str, use_llm: bool = True, budget: float = None, output_file: str = None
) -> None:
    """
    ...
    suggestions = sorted(
        result.get("suggestions", []), key=lambda x: x.get("importance", 0), reverse=True
    )[:5]
...
def enhance_content(
    api_key: str,
    content: str,
    enhancement_type: str = "structure",
    budget: float = None,
...
def main() -> None:
    ...
    parser.add_argument("--type", default="structure", help="Тип улучшения (для action=enhance)")
    ...
    parser.add_argument(
        "--no-llm", action="store_true", help="Отключить использование LLM-компонентов"
    )
    ...
    if not api_key:
        logger.error(
            "API ключ OpenAI не указан. Укажите его через --api-key или переменную окружения OPENAI_API_KEY"
        )
        return
