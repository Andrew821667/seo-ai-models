...
class SchemaOptimizer:
    ...
    def __init__(
        self, schema_url: str = "https://schema.org/version/latest/schemaorg-current-https.jsonld"
    ):
        """
    ...
    def load_schema(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.info(
                    f"Схема Schema.org успешно загружена. Найдено {len(self.schema_types)} типов и {len(self.schema_properties)} доменов свойств."
                )
            else:
                self.logger.warning(
                    f"Не удалось загрузить схему Schema.org. Код ответа: {response.status_code}"
                )
    ...
    def improved_date_processing(self, prop, value, result):
        ...
        try:
            ...
            self.logger.warning(
                f"Не удалось преобразовать дату '{value}' для свойства {prop}: {str(e)}"
            )
    ...
    def _optimize_common_properties(self, schema):
        ...
        for prop in url_properties:
            if prop in schema and schema[prop]:
                ...
                if isinstance(schema[prop], str) and not schema[prop].startswith(
                    ("http://", "https://")
                ):
