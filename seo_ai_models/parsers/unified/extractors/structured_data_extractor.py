...
class StructuredDataExtractor:
    ...
    def extract_all_structured_data(self, html: str, url: Optional[str] = None) -> Dict[str, Any]:
        ...
        structured_data["metadata"] = {
            "total_items": sum(
                len(structured_data.get(t, []))
                for t in structured_data.get("types", [])
                if t != "json_ld"
            )
        }
    ...
    def _extract_definitions(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
        ...
        for dl_idx, dl in enumerate(dl_elements):
            ...
            for dt, dd in zip(dl.find_all("dt"), dl.find_all("dd")):
                dl_items.append(
                    {"term": dt.get_text().strip(), "definition": dd.get_text().strip()}
                )

            if dl_items:
                definitions.append(
                    {"id": f"def_dl_{dl_idx + 1}", "type": "definition_list", "items": dl_items}
                )
