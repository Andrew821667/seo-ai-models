"""
Улучшенный SPA-краулер с усовершенствованными возможностями JavaScript-обработки.
Интегрирует все новые компоненты для эффективного анализа современных веб-приложений.
"""
...
from seo_ai_models.parsers.unified.js_processing.enhanced_websocket_analyzer import (
    EnhancedWebSocketAnalyzer,
)
from seo_ai_models.parsers.unified.js_processing.enhanced_graphql_interceptor import (
    EnhancedGraphQLInterceptor,
)
...
class ImprovedSPACrawler:
    """
    Улучшенный краулер для SPA (Single Page Applications) с продвинутыми возможностями.
    Поддерживает улучшенный анализ WebSocket, GraphQL и клиентскую маршрутизацию.
    """
    ...
    def __init__(
        ...
        user_agent: str = "SEOAIModels ImprovedSPACrawler/1.0",
        ...
        """
        Инициализация улучшенного SPA-краулера.
        ...
        Args:
            ...
            cookies: Предустановленные куки (для авторизации)
        """
        ...
        if self.enable_websocket:
            self.websocket_analyzer = EnhancedWebSocketAnalyzer()

        if self.enable_graphql:
            self.graphql_interceptor = EnhancedGraphQLInterceptor()

        if self.enable_client_routing:
            self.routing_handler = ClientRoutingHandler()

        # Инициализация генератора отпечатков браузера, если включен обход защиты
        if self.bypass_protection:
            self.fingerprint_generator = BrowserFingerprint(consistent=False)

        logger.info(f"ImprovedSPACrawler инициализирован для {base_url} с браузером {browser_type}")

        if enable_websocket:
            logger.info("Включен улучшенный анализ WebSocket")
        if enable_graphql:
            logger.info("Включен улучшенный анализ GraphQL")
    ...
    async def run_crawler(self) -> Dict[str, Any]:
        ...
        async with async_playwright() as p:
            ...
            # Запуск браузера
            browser = await browser_instance.launch(headless=self.headless, args=browser_args)
            ...
            try:
                ...
                if self.bypass_protection:
                    fingerprint = self.fingerprint_generator.get_all_headers()
                    context_options["user_agent"] = fingerprint["User-Agent"]
                ...
                while urls_to_visit and len(self.crawled_urls) < self.max_pages:
                    ...
                    try:
                        ...
                        # Если включен обход защиты, применяем скрипт для обхода обнаружения
                        if self.bypass_protection:
                            await page.add_init_script(
                                self.fingerprint_generator.get_browser_fingerprint_script()
                            )
                        ...
                        if self.enable_client_routing:
                            ...
                            self.routing_handler.detect_router_framework(
                                html_content, window_objects
                            )
                        ...
                        try:
                            ...
                            logger.debug(
                                f"Не удалось закрыть страницу для {current_url}: {str(close_error)}"
                            )
            ...
            try:
                ...
                for selector in self.wait_for_selectors:
                    try:
                        ...
                        logger.debug(
                            f"Селектор '{selector}' не найден в течение {self.wait_for_idle // 2}мс"
                        )
                        continue
            except Exception as e:
    ...
    async def _handle_request(self, route, request):
        ...
        try:
            ...
            if self.enable_graphql and (
                resource_type == "fetch"
                or resource_type == "xhr"
                or url.endswith("/graphql")
                or "graphql" in url
            ):
                try:
    ...
    async def _handle_response(self, response: Response):
        ...
        try:
            ...
            if (
                self.enable_graphql
                and url in self.intercepted_requests
                ...
                try:
                    ...
                    logger.debug(
                        f"Не удалось перехватить GraphQL ответ {url}: {str(gql_response_error)}"
                    )
    ...
    async def _handle_websocket(self, websocket):
        ...
        websocket.on(
            "framesent", lambda payload: self._handle_websocket_frame(payload, "outgoing", url)
        )
        websocket.on(
            "framereceived", lambda payload: self._handle_websocket_frame(payload, "incoming", url)
        )
    ...
    async def _emulate_user_behavior(self, page: Page) -> None:
        ...
        try:
            ...
            for _ in range(3):
                x = viewport_size["width"] * 0.1 + (
                    viewport_size["width"] * 0.8 * (0.2 + 0.6 * (time.time() % 1))
                )
                y = viewport_size["height"] * 0.1 + (
                    viewport_size["height"] * 0.8 * (0.3 + 0.4 * (time.time() % 1))
                )
