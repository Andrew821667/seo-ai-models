...
from seo_ai_models.parsers.unified.analyzers.enhanced_semantic_analyzer import (
    EnhancedSemanticAnalyzer,
)
from seo_ai_models.parsers.unified.utils.parallel_parsing import ParallelParser
from seo_ai_models.parsers.unified.utils.request_utils import (
    fetch_url,
    fetch_url_with_javascript_sync,
)
...
class UnifiedParser(ParserInterface):
    ...
    def __init__(
        ...
        try:
            ...
            self.search_api = SearchAPIIntegration(
                api_keys=search_api_keys, api_provider="serpapi" if search_api_keys else "custom"
            )
    ...
    def detect_site_type(self, url: str) -> Dict[str, Any]:
        ...
        if self.force_spa_mode:
            ...
            return {"is_spa": True, "confidence": 1.0, "detection_method": "forced"}

        if not self.auto_detect_spa:
            logger.info(f"Auto-detection disabled, using standard mode for {url}")
            return {"is_spa": False, "confidence": 1.0, "detection_method": "predefined"}
        ...
        try:
            ...
            if error or not html_content:
                ...
                if error or not html_content:
                    ...
                    return {"is_spa": False, "confidence": 0.0, "error": str(error)}

                # Если JavaScript рендеринг сработал, а обычный запрос нет, скорее всего это SPA
                return {"is_spa": True, "confidence": 0.9, "detection_method": "request_comparison"}
            ...
            return {"is_spa": is_spa, "confidence": confidence, "detection_method": "html_analysis"}

        except Exception as e:
            logger.error(f"Error detecting site type for {url}: {str(e)}")
            return {"is_spa": False, "confidence": 0.0, "error": str(e)}
        ...
        try:
            ...
            if not html_content or status_code != 200:
                ...
                return ParserResult.create_error(
                    f"Failed to fetch URL: {error}", processing_time
                ).to_dict()
            ...
            processing_time = time.time() - start_time
            return ParserResult.create_error(
                f"Error parsing URL: {str(e)}", processing_time
            ).to_dict()
    ...
    def parse_html(self, html: str, base_url: Optional[str] = None, **options) -> Dict[str, Any]:
        ...
        try:
            ...
            processing_time = time.time() - start_time
            return ParserResult.create_error(
                f"Error parsing HTML: {str(e)}", processing_time
            ).to_dict()
    ...
    def crawl_site(self, base_url: str, **options) -> Dict[str, Any]:
        ...
        try:
            ...
            if use_parallel and hasattr(self, "parallel_parser"):
                ...
                parallel_results = self.parallel_parser.parse_urls(crawled_urls, _parse_single_url)
            ...
            processing_time = time.time() - start_time
            return ParserResult.create_error(
                f"Error crawling site: {str(e)}", processing_time
            ).to_dict()
    ...
    def parse_search_results(self, query: str, **options) -> Dict[str, Any]:
        ...
        try:
                    ...
                    if url:
                        try:
                            ...
                            if page_result.get("success", False) and "page_data" in page_result:
                                search_data.results[i]["detailed_analysis"] = page_result[
                                    "page_data"
                                ]
            ...
            processing_time = time.time() - start_time
            return ParserResult.create_error(
                f"Error parsing search results: {str(e)}", processing_time
            ).to_dict()

    def _create_page_data(
        self, url: str, html_content: str, content_data: Dict[str, Any], meta_data: Dict[str, Any]
    ) -> PageData:
        """
        ...
        html_stats = {
            ...
            "text_ratio": (
                char_count / len(html_content) if html_content and len(html_content) > 0 else 0
            ),
    ...
    def _add_page_to_site_data(
        self, site_data: SiteData, url: str, page_result: Dict[str, Any]
    ) -> None:
        """
    ...
    def _calculate_site_statistics(self, pages: Dict[str, PageData]) -> Dict[str, Any]:
        ...
        for page in pages.values():
            for level, texts in page.structure.headings.items():
                if level.startswith("h") and level[1:].isdigit():
                    headings_stats[level + "_count"] = headings_stats.get(
                        level + "_count", 0
                    ) + len(texts)
        ...
        for level in [f"h{i}" for i in range(1, 7)]:
            headings_stats[f"avg_{level}_per_page"] = (
                headings_stats.get(f"{level}_count", 0) / pages_count
            )
        ...
        meta_stats["missing_description_percent"] = (
            meta_stats["missing_description"] / pages_count * 100
        )
        ...
        links_stats = {"total_internal_links": 0, "total_external_links": 0}
        ...
        links_stats["avg_internal_links_per_page"] = (
            links_stats["total_internal_links"] / pages_count
        )
        links_stats["avg_external_links_per_page"] = (
            links_stats["total_external_links"] / pages_count
        )
    ...
    def _build_site_structure(self, pages: Dict[str, PageData]) -> Dict[str, Any]:
        ...
        for url, page_data in pages.items():
            ...
            url_paths[path] = {"url": url, "title": page_data.structure.title}
    ...
    def _generate_simulated_search_results(
        ...
        related_queries = [
            f"{query} {term}" for term in related_terms[: min(7, len(related_terms))]
        ]
