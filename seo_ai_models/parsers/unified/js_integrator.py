...
class JSIntegrator:
    ...
    def get_combined_results(self, parsing_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        results = {
            "summary": {
                "websocket_detected": "websocket" in parsing_result
                and parsing_result["websocket"]["statistics"]["total_messages"] > 0,
                "graphql_detected": "graphql" in parsing_result
                and parsing_result["graphql"]["statistics"]["total_operations"] > 0,
                "client_routing_detected": "routing" in parsing_result
                and parsing_result["routing"]["statistics"]["total_changes"] > 0,
            }
        }
        ...
        if "websocket" in parsing_result and parsing_result["websocket"]["statistics"].get(
            "protocol_stats"
        ):
            for protocol, count in parsing_result["websocket"]["statistics"][
                "protocol_stats"
            ].items():
                if protocol != "raw" and count > 0:
                    technologies.append(
                        {"name": protocol, "type": "websocket_protocol", "count": count}
                    )

        if "graphql" in parsing_result and parsing_result["graphql"]["statistics"].get(
            "detected_clients"
        ):
            for client in parsing_result["graphql"]["statistics"]["detected_clients"]:
                technologies.append({"name": client, "type": "graphql_client"})
