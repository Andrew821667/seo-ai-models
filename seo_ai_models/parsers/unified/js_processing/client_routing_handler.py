...
class ClientRoutingHandler:
    ...
    def detect_router_framework(self, html_content: str, window_objects: Optional[List[str]] = None) -> Optional[Dict[str, Any]]:
        """
        ...
        for signature in router_signatures:
            for pattern in signature["html_patterns"]:
                if re.search(pattern, html_content, re.IGNORECASE):
                    ...
                    self.logger.info(f"Обнаружен фреймворк маршрутизации: {framework['name']} (уверенность: {framework['confidence']})")
                    return framework
    ...
    def record_route_change(self, from_url: str, to_url: str, 
                          state_object: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
