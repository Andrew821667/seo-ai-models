...
class EnhancedWebSocketAnalyzer:
    ...
    PROTOCOLS = {
        ...
        "raw": {
            "patterns": [
                r'^.*$'  # Всегда совпадает, используется как последнее средство
            ],
            "identifiers": []
        }
    }
    ...
    def register_pattern(self, name: str, key_fields: List[str], value_fields: List[str], 
                       type_identifier: Optional[str] = None, protocol: Optional[str] = None) -> None:
        """
    ...
    def capture_message(self, message_data: Union[str, bytes, Dict, List], direction: str = "incoming", 
                      endpoint: Optional[str] = None, opcode: Optional[int] = None) -> Optional[WebSocketMessage]:
        """
    ...
    def _process_message(self, message: WebSocketMessage) -> None:
        ...
        for pattern in self.patterns:
            ...
            if pattern.get("protocol") and message.protocol and pattern["protocol"] != message.protocol:
                continue
    ...
    def _extract_by_pattern(self, content: Any, pattern: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
    ...
    def get_active_connections(self) -> List[Dict[str, Any]]:
        ...
        for endpoint, info in self.connection_info.items():
            ...
            connections.append({
                ...
                "last_activity": datetime.fromtimestamp(info.get("last_activity", 0)).isoformat(),
                "is_active": is_active,
