...
class SearchAPIIntegration:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    logger.info(
                        f"Retrying in {self.retry_delay}s ({attempt + 1}/{self.max_retries})"
                    )
    ...
    def _search_with_serpapi(
        self, query: str, api_key: str, proxy: Optional[str], **kwargs
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "search_information": {
                ...
                "time_taken_displayed": data.get("search_information", {}).get(
        ...
        return {
            ...
            "people_also_ask": [
                {"question": f"What is {query}?", "snippet": f"Sample answer about {query}..."},
                {
                    ...
                    "snippet": f"Sample guide about using {query}...",
                },
                {
                    "question": f"Why is {query} important?",
                    "snippet": f"Sample explanation about importance of {query}...",
                },
            ],
            "search_features": {
