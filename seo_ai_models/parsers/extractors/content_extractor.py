...
class ContentExtractor:
    ...
    def extract_content(self, html_content: str, url: Optional[str] = None) -> Dict[str, Any]:
        ...
        for comment in soup.find_all(
            text=lambda text: isinstance(text, NavigableString) and text.strip().startswith("<!--")
        ):
            comment.extract()
        ...
        for tag in ["main", "article", "div", "section"]:
            ...
            for element in elements:
                ...
                if (
                    text_length > self.min_content_length
        ...
        for list_tag in soup.find_all(["ul", "ol"]):
            if not self._is_excluded_element(list_tag):
                ...
                if list_items:
                    lists.append({"type": list_tag.name, "items": list_items})
