...
def add_caching_support(pipeline_class):
    ...
    def __init_with_cache(self, *args, **kwargs):
        ...
        self.cache_manager = CacheManager(
            cache_dir=cache_dir, max_age=cache_max_age, enabled=cache_enabled
        )
    ...
    def analyze_url_with_cache(self, url, detect_type=True, use_cache=True):
        ...
        if use_cache:
            # Параметры для кэша
            cache_params = {"detect_type": detect_type, "force_spa_mode": self.force_spa_mode}
    ...
    def detect_site_type_with_cache(self, url, use_cache=True):
        ...
        if use_cache:
            # Параметры для кэша
            cache_params = {"force_spa_mode": self.force_spa_mode}
