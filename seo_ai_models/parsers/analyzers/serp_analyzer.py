    ...
    def analyze_top_results(self, query: str) -> Dict[str, Any]:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    content_data = self.content_extractor.extract_content(
                        response.text, result["url"]
                    )
                    meta_data = self.meta_extractor.extract_meta_information(
                        response.text, result["url"]
                    )
                    ...
                    logger.warning(
                        f"Не удалось получить контент с {result['url']}: HTTP {response.status_code}"
                    )
        ...
        analysis = {
            ...
            "heading_counts": {
                ...
                for h_level, counts in heading_counts.items()
                if counts
            },
        }
        ...
        analysis["common_words"] = {"titles": common_title_words, "snippets": common_snippet_words}
        ...
        if self.search_engine == "google":
            ...
            try:
                ...
                for div in related_divs:
                    ...
                    if (
                        text and text != query and len(text) < 100
                    ):  # Простая эвристика для связанных запросов
