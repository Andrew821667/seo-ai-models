...
class DashboardConfig:
    ...
    def __init__(
        ...
        self.data_dir = data_dir or os.path.join(
            os.path.expanduser("~"), ".seo_ai_models", "dashboard"
        )
        self.user_settings = {}
...
class DashboardWidget:
    ...
    def __init__(
        self, widget_id: str, title: str, type: str, config: Optional[Dict[str, Any]] = None
    ):
        """
...
class DashboardView:
    ...
    def __init__(
        self, view_id: str, title: str, icon: str = "", description: str = "", order: int = 0
    ):
        """
...
class DashboardUI:
                    ...
                    "items": [
                        {
                            ...
                            "value_key": "total_projects",
                            "icon": "folder",
                        },
                        {
                            "label": "Активных проектов",
                            "value_key": "active_projects",
                            "icon": "folder-open",
                        },
                        {
                            "label": "Всего анализов",
                            "value_key": "total_analyses",
                            "icon": "analytics",
                        },
                        {
                            "label": "Активных анализов",
                            "value_key": "active_analyses",
                            "icon": "trending-up",
                        },
                        {
                            "label": "Завершенных анализов",
                            "value_key": "completed_analyses",
                            "icon": "check-circle",
                        },
                        {
                            "label": "Всего отчетов",
                            "value_key": "total_reports",
                            "icon": "file-text",
                        },
                    ],
                },
        ...
        users_view.add_widget(
            DashboardWidget(
                ...
                config={
                    ...
                    "actions": [{"id": "terminate", "label": "Завершить", "icon": "x-circle"}],
                    "pagination": True,
                    "sorting": True,
                },
            )
            ...
            DashboardWidget(
                ...
                config={
                    "fields": [
                        ...
                        {
                            "name": "default_views",
                            "label": "Представления по умолчанию",
                            "type": "multi_select",
                            "options": [
    ...
    def _refresh_overview_data(self):
        ...
        overview_stats_widget = next(
            (w for w in view.widgets if w.widget_id == "overview_stats"), None
        )
        ...
        recent_analyses_widget = next(
            (w for w in view.widgets if w.widget_id == "recent_analyses"), None
        )
        ...
        recent_reports_widget = next(
            (w for w in view.widgets if w.widget_id == "recent_reports"), None
        )
        if recent_reports_widget:
            ...
            for report in recent_reports:
                project = (
                    self.project_management.get_project(report.project_id)
        ...
        activity_chart_widget = next(
            (w for w in view.widgets if w.widget_id == "activity_chart"), None
        )
    ...
    def _refresh_projects_data(self):
        ...
        projects_list_widget = next(
            (w for w in view.widgets if w.widget_id == "projects_list"), None
        )
    ...
    def _refresh_reports_data(self):
        ...
        templates_list_widget = next(
            (w for w in view.widgets if w.widget_id == "templates_list"), None
        )
    ...
    def _refresh_users_data(self):
        ...
        active_sessions_widget = next(
            (w for w in view.widgets if w.widget_id == "active_sessions"), None
        )
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        # Получаем статистику по проектам
        project_stats = (
            self.project_management.get_project_statistics() if self.project_management else {}
        )

        # Получаем статистику по отчетам
        report_stats = (
            self.report_generator.get_report_statistics() if self.report_generator else {}
        )
    ...
    def generate_dashboard_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "views": [
                view.to_dict() for view in sorted(self.views.values(), key=lambda v: v.order)
            ],
    ...
    def create_project(
        self, name: str, url: str, description: str = "", settings: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        ...
        project = self.project_management.create_project(
            name=name, url=url, description=description, settings=settings
        )
