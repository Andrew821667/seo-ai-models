...
class ReportGenerator:
    ...
    def __init__(self, data_dir: Optional[str] = None, api_client=None, project_management=None):
        ...
        self.data_dir = data_dir or os.path.join(
            os.path.expanduser("~"), ".seo_ai_models", "reports"
        )
            ...
            for section in template.structure.get("sections", []):
                ...
                if content_type == "summary":
                    data["content"][section_id] = {
                        "summary_text": analysis.results.get(
                            "summary", "Сводка по анализу отсутствует"
                        ),
    ...
    def _generate_executive_summary(
        self, analysis_data: Dict[str, Any], project_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            key_metrics = {
                ...
                "critical_issues": len(
                    [i for i in analysis_data.get("issues", []) if i.get("severity") == "critical"]
                ),
            }
    ...
    def get_recent_reports(self, limit: int = 10) -> List[Report]:
        ...
        sorted_reports = sorted(self.reports.values(), key=lambda x: x.updated_at, reverse=True)
