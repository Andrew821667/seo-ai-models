...
class CMSConnector(ABC):
    ...
    def _make_request(
        ...
        headers = headers or {}
        headers.update(
            {"User-Agent": "SEO AI Models CMS Connector/1.0", "Accept": "application/json"}
        )
        ...
        try:
            ...
            if not response.text or not response.headers.get("Content-Type", "").startswith(
                "application/json"
            ):
...
class WordPressConnector(CMSConnector):
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            self._make_request(
                method="GET", endpoint=self.api_base, auth=(self.username, self.password)
            )
            return True
        except requests.RequestException:
            return False
    ...
    def get_pages(self, params: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        params = params or {}
        default_params = {"per_page": 100, "status": "publish"}
    ...
    def get_cms_info(self) -> Dict[str, Any]:
        ...
        try:
            response = self._make_request(
                method="GET", endpoint="wp-json", headers=self._get_auth_headers()
            )
            ...
            return {"cms_type": "wordpress", "error": str(e)}
...
class DrupalConnector(CMSConnector):
    ...
    def authenticate(self) -> bool:
        ...
        try:
            response = self.session.get(
                f"{self.site_url}/session/token", verify=self.verify_ssl, timeout=self.timeout
            )
            response.raise_for_status()
        ...
        try:
            response = self._make_request(
                method="POST",
                endpoint="user/login",
                data={"name": self.username, "pass": self.password},
    ...
    def update_page_content(self, page_id: str, content: str, title: Optional[str] = None) -> bool:
        ...
        # Формируем данные для обновления
        data = {"data": {"type": "node--page", "id": page_id, "attributes": {}}}

        # Если указан контент, обновляем его
        if content:
            data["data"]["attributes"]["body"] = {"value": content, "format": "full_html"}
        ...
        try:
            response = self._make_request(
                ...
                endpoint=f"{self.api_base}/node/page/{page_id}",
                data=data,
                headers={**self._get_auth_headers(), "Content-Type": "application/vnd.api+json"},
            )
    ...
    def get_cms_info(self) -> Dict[str, Any]:
        ...
        try:
            response = self._make_request(
                method="GET", endpoint=self.api_base, headers=self._get_auth_headers()
            )
            ...
            return {"cms_type": "drupal", "error": str(e)}


class JoomlaConnector(CMSConnector):
    """Коннектор для Joomla с использованием Joomla Web Services API."""

    def __init__(self, site_url: str, api_key: str, verify_ssl: bool = True, timeout: int = 30):
        """
    ...
    def get_pages(self, params: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        # Объединяем параметры по умолчанию с дополнительными
        params = params or {}
        default_params = {"limit": 100, "state": 1}  # Опубликованные статьи
        params = {**default_params, **params}
    ...
    def get_cms_info(self) -> Dict[str, Any]:
        ...
        try:
            response = self._make_request(
                method="GET", endpoint=f"{self.api_base}/config", headers=self._get_auth_headers()
            )
            ...
            return {"cms_type": "joomla", "error": str(e)}
...
class BitrixConnector(CMSConnector):
        ...
        if self.username and self.password:
            try:
                ...
                login_page = self.session.get(
                    f"{self.site_url}/auth/", verify=self.verify_ssl, timeout=self.timeout
                )
    ...
    def get_pages(self, params: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        try:
            response = self._make_request(
                method="POST", endpoint=f"{self.api_base}/iblock.element.get", data=api_params
            )
    ...
    def get_page_content(self, page_id: str) -> Dict[str, Any]:
        ...
        try:
            response = self._make_request(
                ...
                data={"id": page_id, **self._get_auth_params()},
            )
    ...
    def update_page_content(self, page_id: str, content: str, title: Optional[str] = None) -> bool:
        ...
        try:
            response = self._make_request(
                ...
                data={"id": page_id, "fields": fields, **self._get_auth_params()},
            )
    ...
    def get_cms_info(self) -> Dict[str, Any]:
        ...
        # Аутентифицируемся, если еще не сделали этого
        if not self.authenticate():
            return {"cms_type": "1c-bitrix", "error": "Failed to authenticate"}
        ...
        try:
            ...
            return {"version": response.get("result", ""), "cms_type": "1c-bitrix"}
        except requests.RequestException as e:
            logger.error(f"Error getting 1C-Bitrix info: {str(e)}")
            return {"cms_type": "1c-bitrix", "error": str(e)}
    ...
    def detect_cms(site_url: str) -> str:
        ...
        try:
            ...
            response = requests.get(
                site_url, headers={"User-Agent": "SEO AI Models CMS Detector/1.0"}, timeout=10
            )
            response.raise_for_status()
            ...
            if (
                "bitrix/js" in html
                or "бизнес-процессы, устанавливаемая на Интранет-портал Битрикс24" in html
            ):
                return "bitrix"
                ...
                for path in paths:
                    try:
                        ...
                        check_response = requests.head(
                            check_url,
                            headers={"User-Agent": "SEO AI Models CMS Detector/1.0"},
                            timeout=5,
                        )
