...
class DemoPhase5:
    ...
    def demo_freemium_model(self):
        ...
        freemium_advisor = FreemiumAdvisor(plan=FreemiumPlan.FREE, user_id=self.user_id)

        # Анализируем контент с учетом ограничений бесплатного плана
        sample_content = (
            "Это пример контента для анализа SEO. Оптимизация для поисковых систем очень важна."
        )
        ...
        logger.info(
            f"Результат анализа для бесплатного плана: {'Ограниченный' if 'limited_info' in analysis_result and analysis_result['limited_info'] else 'Полный'}"
        )
        ...
        logger.info(
            f"Доступные планы для обновления: {[upgrade['code'] for upgrade in available_upgrades]}"
        )

        # Имитируем процесс обновления до плана Micro
        upgrade_result = upgrade_path.initiate_upgrade(
            target_plan=FreemiumPlan.MICRO, payment_info={"card_last4": "1234", "amount": 1990}
        )
        ...
        value_demonstrator = ValueDemonstrator(current_plan=FreemiumPlan.FREE, user_id=self.user_id)

        # Демонстрируем функцию полных рекомендаций
        demo_result = value_demonstrator.demonstrate_feature(
            feature_name="full_recommendations", content=sample_content
        )

        # Выводим информацию о демонстрации
        logger.info(f"Демонстрация функции 'Полные рекомендации':")
        logger.info(
            f"  Количество рекомендаций в бесплатном плане: {demo_result['demo_result']['free_plan']['recommendations_count']}"
        )
        logger.info(
            f"  Количество рекомендаций в платном плане: {demo_result['demo_result']['paid_plan']['recommendations_count']}"
        )
        logger.info(
            f"  Улучшение: {demo_result['demo_result']['comparison']['improvement_percentage']:.1f}%"
        )
        ...
        onboarding_wizard = OnboardingWizard(user_id=self.user_id, plan="free")
        ...
        step_manager = StepManager(user_id=self.user_id, plan="free")

        # Получаем оптимальный путь онбординга
        onboarding_path = step_manager.get_onboarding_path(include_optional=False)
        logger.info(
            f"Оптимальный путь онбординга (только обязательные шаги): {[step['id'] for step in onboarding_path]}"
        )
        ...
        tutorial_generator = TutorialGenerator(
            user_id=self.user_id, plan="free", experience_level="beginner"
        )
        ...
        logger.info(
            f"Руководство быстрого старта содержит {len(quick_start_guide['quick_start_guide']['materials'])} материалов"
        )
        logger.info(
            f"Общее время изучения: {quick_start_guide['quick_start_guide']['total_time']} минут"
        )
    ...
    def demo_scaling_components(self):
        ...
        analysis_result = performance_optimizer.analyze_component(
            test_function, "test_function", test_data=5
        )
        logger.info(
            f"Результат анализа компонента: среднее время выполнения {analysis_result['average_execution_time']:.4f} сек"
        )
        ...
        for i in range(5):
            task_result = distributed_processing.add_task(
                function_name="process_data", args=[i], priority=TaskPriority.NORMAL
            )
        ...
        system_monitor = SystemMonitor(system_metrics_interval=2, application_metrics_interval=2)
        ...
        if application_metrics["status"] == "success" and application_metrics["metrics"]:
            ...
            if "performance" in metrics and "avg_processing_time" in metrics["performance"]:
                logger.info(
                    f"  Среднее время обработки: {metrics['performance']['avg_processing_time']} мс"
                )
        ...
        if component_status["status"] == "success":
            logger.info(
                f"  Текущее количество экземпляров: {component_status['component']['current_instances']}"
            )
