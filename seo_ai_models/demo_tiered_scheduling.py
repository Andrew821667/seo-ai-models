...
class SimpleDemo:
    ...
    def demo_credit_system(self):
        ...
        logger.info(
            f"Использовано 50 кредитов типа {CreditType.ANALYSIS.value} для анализа контента"
        )
        logger.info(
            f"Использовано 30 кредитов типа {CreditType.KEYWORD.value} для анализа ключевых слов"
        )
    ...
    def demo_budget_planning(self):
        ...
        budget_allocation = {
            "content_analysis": {CreditType.ANALYSIS.value: 200, CreditType.LLM.value: 0},
            "keyword_analysis": {CreditType.ANALYSIS.value: 150, CreditType.KEYWORD.value: 200},
            "llm_analysis": {CreditType.ANALYSIS.value: 50, CreditType.LLM.value: 150},
        }
    ...
    def demo_cost_optimization(self):
        ...
        for i, rec in enumerate(recommendations):
            ...
            logger.info(
                f"   Оценка экономии: {rec['estimated_savings']} кредитов типа {rec['credit_type']}"
            )
