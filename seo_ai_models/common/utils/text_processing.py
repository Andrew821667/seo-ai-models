...
class TextProcessor:
    ...
    def tokenize(
        self, text: str, remove_stopwords: bool = False, language: str = None
    ) -> List[str]:
        ...
        for line in text.split("\n"):
            ...
            if line.startswith("#"):
                ...
                if header_text and level <= 6:  # Проверяем валидность заголовка
                    headers.append({"level": level, "text": header_text, "type": "markdown"})
    ...
    def extract_keywords(
        self, text: str, max_keywords: int = 10, min_word_length: int = 4, language: str = None
        ...
        if max_freq > 0:
            keywords = [
                (word, count / max_freq) for word, count in counter.most_common(max_keywords)
            ]
        else:
            keywords = []
    ...
    def extract_ngrams(
        self, text: str, n: int = 2, max_ngrams: int = 10, language: str = None
    ) -> List[Tuple[str, float]]:
        ...
        if max_freq > 0:
            top_ngrams = [
                (ngram, count / max_freq) for ngram, count in counter.most_common(max_ngrams)
            ]
        else:
            top_ngrams = []
