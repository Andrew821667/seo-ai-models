...
def demo_parsing_pipeline(args):
    ...
    pipeline = ParsingPipeline(
        delay=args.delay, respect_robots=not args.ignore_robots, search_engine=args.search_engine
    )
    ...
    if args.mode == "url":
        ...
        if args.url:
            results = pipeline.crawl_and_analyze_site(args.url, max_pages=args.max_pages)
        else:
            results = {"error": "No URL specified"}
        ...
        if args.query:
            results = pipeline.analyze_keyword(
                args.query, analyze_competitors=not args.skip_competitors
            )
        else:
            results = {"error": "No query specified"}
...
def main():
    ...
    serp_parser.add_argument(
        "--search-engine", choices=["google", "bing"], default="google", help="Search engine to use"
    )
    ...
    pipeline_parser.add_argument(
        "--mode", choices=["url", "site", "keyword"], required=True, help="Analysis mode"
    )
    ...
    pipeline_parser.add_argument(
        "--max-pages", type=int, default=10, help="Maximum pages to crawl (for site mode)"
    )
    ...
    pipeline_parser.add_argument(
        "--skip-competitors",
        action="store_true",
        help="Skip competitor analysis (for keyword mode)",
    )
