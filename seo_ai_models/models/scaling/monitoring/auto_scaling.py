...
class CPUUtilizationPolicy:
    ...
    def check(self, metrics: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        return self._handle_no_scaling_needed(metrics)
    ...
    def _handle_cooldown_check(self, metrics: Dict[str, Any]) -> None:
        """Обрабатывает проверку периода охлаждения."""
        logger.debug(f"Cooldown period active for {self.component}")
        return None

    def _handle_no_metrics(self, metrics: Dict[str, Any]) -> None:
        """Обрабатывает отсутствие CPU метрик."""
        logger.warning(f"No CPU metrics available for {self.component}")
        return None

    def _handle_no_scaling_needed(self, metrics: Dict[str, Any]) -> None:
        """Обрабатывает случай когда масштабирование не требуется."""
        cpu_util = metrics.get("cpu", {}).get("percent", 0)
        logger.debug(f"No scaling needed for {self.component}, CPU: {cpu_util}%")
        return None
...
class AutoScaling:
    ...
    def add_component(
        self, name: str, min_instances: int = 1, max_instances: int = 10, current_instances: int = 1
    ) -> Dict[str, Any]:
        """
        ...
        with self.lock:
            if name in self.components:
                return {"status": "error", "message": f"Component {name} already exists"}
    ...
    def get_component_status(self, name: str) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            return {"status": "success", "component": self.components[name]}
    ...
    def scale_component(self, component_name: str, action: str, reason: str) -> Dict[str, Any]:
        """
    ...
    def get_scaling_history(
        ...
        with self.lock:
            ...
            if component_name:
                filtered_history = [
                    entry for entry in self.scaling_history if entry["component"] == component_name
                ]
            else:
    ...
    def add_policy(self, policy: Any) -> Dict[str, Any]:
        ...
        with self.lock:
            if policy.name in self.policies:
                return {"status": "error", "message": f"Policy {policy.name} already exists"}

            # Проверяем, существует ли компонент
            if policy.component not in self.components:
                return {"status": "error", "message": f"Component {policy.component} not found"}
    ...
    def get_policy(self, name: str) -> Dict[str, Any]:
        ...
        with self.lock:
            if name not in self.policies:
                return {"status": "error", "message": f"Policy {name} not found"}

            return {"status": "success", "policy": self.policies[name].to_dict()}
