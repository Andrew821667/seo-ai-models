    ...
    def collect(self) -> Dict[str, Any]:
        ...
        with self.counters_lock:
            ...
            if counters_copy["processing_time_count"] > 0:
                avg_processing_time = counters_copy["processing_time_sum"] / counters_copy["processing_time_count"]
            
            # Рассчитываем процент успешных запросов
            success_rate = None
            if counters_copy["requests"] > 0:
                success_rate = (counters_copy["successful_requests"] / counters_copy["requests"]) * 100
    ...
    def collect(self) -> Dict[str, Any]:
        ...
        with self.counters_lock:
            ...
            if counters_copy["queries"] > 0:
                success_rate = (counters_copy["successful_queries"] / counters_copy["queries"]) * 100
...
class SystemMonitor:
    ...
    def load_metrics_from_file(
        ...
        if not os.path.exists(file_path):
            return {
                "status": "error",
                "message": f"Metrics file {file_path} not found"
            }
