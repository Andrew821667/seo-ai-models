                ...
                if function is None:
                    ...
                    try:
                        ...
                        if task.timeout is not None:
                            timeout_thread = threading.Thread(target=self._function_with_timeout, args=(task, function))
...
class DistributedProcessing:
    ...
    def add_worker(
        ...
        if worker_id in self.workers:
            return {
                "status": "error",
                "message": f"Worker {worker_id} already exists"
            }
        ...
        return {
            "status": "success",
            "message": f"Worker {worker_id} added",
            "worker_id": worker_id
        }
    ...
    def remove_worker(self, worker_id: str) -> Dict[str, Any]:
        ...
        return {
            "status": "success",
            "message": f"Worker {worker_id} removed"
        }
    ...
    def get_active_tasks_by_worker(self, worker_id: str) -> Dict[str, Any]:
        ...
        return {
            "worker_id": worker_id,
            "active_tasks": self.workers[worker_id].get_active_tasks()
        }
    ...
    def clear_completed_tasks(self, clear_failed: bool = True) -> Dict[str, Any]:
        ...
        with self.tasks_lock:
            ...
            for task_id, task in self.tasks.items():
                if task.status == TaskStatus.COMPLETED or (clear_failed and task.status == TaskStatus.FAILED):
            ...
            return {
                "status": "success",
                "message": f"Cleared {len(tasks_to_remove)} tasks"
            }
