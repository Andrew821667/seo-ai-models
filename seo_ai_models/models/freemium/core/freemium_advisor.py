...
class FreemiumAdvisor(TieredAdvisor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.value_demonstrator = ValueDemonstrator(
            current_plan=self.freemium_plan, user_id=user_id
        )
    ...
    def _limit_free_plan_results(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "recommendations" in result:
            limited_result["recommendations"] = result["recommendations"][
                :3
            ]  # Только 3 рекомендации
    ...
    def complete_upgrade(self, target_plan: Union[str, FreemiumPlan]) -> Dict[str, Any]:
        ...
        if result.get("status") == "success":
            ...
            self.value_demonstrator = ValueDemonstrator(
                current_plan=self.freemium_plan, user_id=self.user_id
            )
    ...
    def _get_onboarding_step(self, step: str) -> Dict[str, Any]:
        ...
        return steps.get(step, {"error": "Invalid step", "message": f"Step {step} not found"})
    ...
    def get_remaining_quota(self) -> Dict[str, Any]:
        ...
        return {"plan": self.freemium_plan.value, "message": "No quota limits for this plan"}

    def _track_feature_usage(
        self, feature_name: str, user_id: str = None, usage_data: Dict[str, Any] = None
    ) -> None:
        """Отслеживает использование функций пользователем."""
        try:
            import time

            if not hasattr(self, "_feature_usage_stats"):
                self._feature_usage_stats = {}

            if user_id not in self._feature_usage_stats:
                self._feature_usage_stats[user_id] = {
                    "total_usage_count": 0,
                    "features": {},
                    "first_usage_time": time.time(),
                }

            user_stats = self._feature_usage_stats[user_id]
            if feature_name not in user_stats["features"]:
                user_stats["features"][feature_name] = {"usage_count": 0}

            user_stats["features"][feature_name]["usage_count"] += 1
            user_stats["total_usage_count"] += 1

        except Exception as e:
            import logging

            logging.getLogger(__name__).error(f"Error tracking feature usage: {e}")

    def _suggest_upgrade(
        self, user_id: str = None, context: str = None
    ) -> Optional[Dict[str, Any]]:
        """Генерирует предложения по апгрейду."""
        try:
            if (
                not hasattr(self, "_feature_usage_stats")
                or user_id not in self._feature_usage_stats
            ):
                return None

            user_stats = self._feature_usage_stats[user_id]
            if user_stats["total_usage_count"] > 50:
                import time
                ...
                return {
                    ...
                    "urgency_score": 60,
                    "reasons": [
                        f"Высокая активность: {user_stats['total_usage_count']} использований"
                    ],
                    "context": context,
                    "timestamp": time.time(),
                }
            ...
            logging.getLogger(__name__).error(f"Error generating upgrade suggestion: {e}")
            return None
