...
class QuotaManager:
    ...
    def __init__(
        ...
        self.storage_path = storage_path or os.path.join(
            os.path.expanduser("~"), ".seo_ai_models", "quotas"
        )
    ...
    def _initialize_user_quotas(self) -> Dict[str, Any]:
        ...
        for operation, limit in self.DEFAULT_QUOTAS.items():
            reset_date = (
                datetime.now() + timedelta(days=self.QUOTA_PERIODS[operation])
            ).isoformat()

            quotas["quotas"][operation] = {"limit": limit, "used": 0, "reset_date": reset_date}
        ...
        for operation, quota_info in self.user_quotas["quotas"].items():
            ...
            if now >= reset_date:
                ...
                quota_info["reset_date"] = (
                    now + timedelta(days=self.QUOTA_PERIODS[operation])
                ).isoformat()
                updated = True
    ...
    def reset_quota(self, operation: Optional[str] = None):
        ...
        if operation:
            ...
            for op, quota_info in self.user_quotas["quotas"].items():
                quota_info["used"] = 0
                quota_info["reset_date"] = (
                    datetime.now() + timedelta(days=self.QUOTA_PERIODS[op])
                ).isoformat()
