...
class EEATAnalyzer:
    ...
    def analyze(
        ...
        result = {
            ...
            "component_details": {
                "expertise": {
                    "found_markers": self._count_markers(text_lower, self.expertise_markers)
                },
                "authority": {
                    "found_markers": self._count_markers(text_lower, self.authority_markers)
                },
                "trust": {"found_markers": self._count_markers(text_lower, self.trust_markers)},
                "experience": {
                    "found_markers": self._count_markers(text_lower, self.experience_markers)
                },
            },
        }
    ...
    def _evaluate_expertise(self, text: str) -> float:
        ...
        has_professional_status = bool(
            re.search(r"(сертифицированный|лицензированный|аккредитованный)", text)
        )
    ...
    def _evaluate_authority(self, text: str) -> float:
        ...
        authoritative_sources = ["исследование", "университет", "журнал", "публикация", "институт"]
    ...
    def _generate_recommendations(
        ...
        if (
            is_ymyl
            and (experience_score + expertise_score + authority_score + trust_score) / 4 < 0.6
        ):
