...
class EnhancedEEATAnalyzer(EEATAnalyzer):
    ...
    def _prepare_features_for_model(
        self, text_lower: str, base_metrics: Dict[str, float]
    ) -> np.ndarray:
        ...
        has_sources = (
            ("источник" in text_lower)
            or ("reference" in text_lower)
        ...
        has_author = (
            ("автор" in text_lower)
            or ("профессор" in text_lower)
            or ("доктор" in text_lower)
            or ("эксперт" in text_lower)
        )
        ...
        assert (
            len(features) == 8
        ), f"Ошибка: вектор признаков должен содержать 8 элементов, сейчас: {len(features)}"
