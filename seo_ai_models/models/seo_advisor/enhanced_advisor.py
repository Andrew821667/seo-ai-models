...
from seo_ai_models.models.seo_advisor.analyzers.enhanced_content_analyzer import (
    EnhancedContentAnalyzer,
)
from seo_ai_models.models.seo_advisor.analyzers.semantic_analyzer import SemanticAnalyzer
from seo_ai_models.models.seo_advisor.analyzers.eeat.enhanced_eeat_analyzer import (
    EnhancedEEATAnalyzer,
)
from seo_ai_models.models.seo_advisor.predictors.calibrated_rank_predictor import (
    CalibratedRankPredictor,
)
...
class EnhancedSEOAdvisor(SEOAdvisor):
    ...
    def analyze_content(
        ...
        use_llm: bool = True,
        llm_budget: Optional[float] = None,
        **kwargs,
    ) -> Dict[str, Any]:
        """
    ...
    def _analyze_with_llm(self, content: str, budget: Optional[float] = None) -> Dict[str, Any]:
        ...
        if eeat_analyzer:
            try:
                eeat_result = eeat_analyzer.analyze_eeat(content, budget=component_budget)
        ...
        total_cost = sum(
            result.get("cost", 0) for result in llm_result.values() if isinstance(result, dict)
        )
    ...
    def _merge_results(
        self, base_result: Dict[str, Any], llm_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if "eeat" in llm_result and "eeat" in result:
            result["eeat"] = self._merge_eeat_data(result["eeat"], llm_result["eeat"])
    ...
    def _merge_semantic_data(
        self, base_semantic: Dict[str, Any], llm_semantic: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _merge_eeat_data(
        self, base_eeat: Dict[str, Any], llm_eeat: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _extract_llm_metrics(self, llm_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        metrics = {"total_cost": llm_result.get("total_cost", 0)}

        # Добавляем оценки совместимости
        if "compatibility" in llm_result:
            for factor, score in (
                llm_result["compatibility"].get("compatibility_scores", {}).items()
            ):
    ...
    def _update_scores_and_suggestions(self, result: Dict[str, Any]) -> None:
        ...
        if "overall_score" in result and "llm_metrics" in result:
            ...
            llm_scores = [
                score for score in [llm_compatibility, llm_citability, llm_eeat] if score > 0
            ]
            if llm_scores:
        ...
        if "suggestions" in result and "llm_analysis" in result:
            ...
            result["suggestions"] = sorted(
                result["suggestions"], key=lambda x: x.get("importance", 0), reverse=True
            )
    ...
    def _create_llm_suggestions(self, llm_analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if "compatibility" in llm_analysis:
            for category, improvements in (
                llm_analysis["compatibility"].get("suggested_improvements", {}).items()
            ):
        ...
        if "citability" in llm_analysis:
            for category, improvements in (
                llm_analysis["citability"].get("suggested_improvements", {}).items()
            ):
                for improvement in improvements:
                    suggestions.append(
                        {
        ...
        if "eeat" in llm_analysis:
            for category, improvements in (
                llm_analysis["eeat"].get("suggested_improvements", {}).items()
            ):
                for improvement in improvements:
                    suggestions.append(
                        {
    ...
    def enhance_content(
        self, content: str, enhancement_type: str = "structure", budget: Optional[float] = None
    ) -> Dict[str, Any]:
        """
    ...
    def analyze_content_for_llm(
        self, content: str, llm_type: str = "generic", budget: Optional[float] = None
    ) -> Dict[str, Any]:
        """
        ...
        # Проверяем доступность LLM-компонентов
        if not self.llm_enabled:
            return {"success": False, "error": "LLM-компоненты недоступны"}
        ...
        result = {"success": True, "llm_type": llm_type, "content_length": len(content)}
        ...
        if "compatibility" in result:
            for category, improvements in (
                result["compatibility"].get("suggested_improvements", {}).items()
            ):
                for improvement in improvements:
                    suggestions.append(
                        {"type": "compatibility", "category": category, "description": improvement}
                    )

        # Из цитируемости
        if "citability" in result:
            for category, improvements in (
                result["citability"].get("suggested_improvements", {}).items()
            ):
                for improvement in improvements:
                    suggestions.append(
                        {"type": "citability", "category": category, "description": improvement}
                    )
        ...
        if "eeat" in result:
            for category, improvements in result["eeat"].get("suggested_improvements", {}).items():
                for improvement in improvements:
                    suggestions.append(
                        {"type": "eeat", "category": category, "description": improvement}
                    )
        ...
        if "compatibility" in result:
            compatibility_score = (
                result["compatibility"].get("compatibility_scores", {}).get("overall", 0)
            )
