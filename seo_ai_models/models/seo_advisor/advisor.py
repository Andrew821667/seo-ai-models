...
class SEOAdvisor:
    ...
    def analyze_content(self, content: str, target_keywords: List[str]) -> SEOAnalysisReport:
        ...
        enhanced_recommendations = self.suggester.generate_suggestions(
            base_recommendations, prediction["feature_scores"], self.industry
        )

        # Получаем семантические рекомендации
        semantic_recommendations = self.semantic_analyzer.generate_recommendations(
            semantic_analysis
        )
        ...
        priorities = self.suggester.prioritize_tasks(
            enhanced_recommendations, prediction["feature_scores"], prediction["weighted_scores"]
        )
    ...
    def _evaluate_content_quality(
        self, metrics: Dict[str, float], keyword_analysis: Dict[str, Union[float, Dict]]
        ...
        scores["overall_quality"] = self._calculate_overall_quality(metrics)

        # Анализ ключевых слов
        scores["keyword_score"] = keyword_analysis.get("relevance", 0)

        # Определение сильных сторон
        if scores["overall_quality"] > 0.8:
            strengths.append("Высокое общее качество контента")
        if metrics.get("semantic_density", 0) > 0.7:
            strengths.append("Отличная семантическая плотность")
        if metrics.get("readability", 0) > 0.75:
            strengths.append("Высокая читаемость")

        # Определение слабых сторон
        if scores["overall_quality"] < 0.5:
            weaknesses.append("Низкое общее качество контента")
        if metrics.get("keyword_density", 0) < 0.3:
            weaknesses.append("Недостаточная плотность ключевых слов")

        # Генерация улучшений
        if "semantic_coverage" in metrics and metrics["semantic_coverage"] < 0.6:
            improvements.append("Улучшить семантическое покрытие темы")
        if scores["keyword_score"] < 0.5:
            improvements.append("Оптимизировать использование ключевых слов")
