...
class LightweightOptimizer:
    ...
    def optimize_content(
        ...
        **kwargs
        ...
        if any(r.get('category') == 'readability' for r in prioritized_recommendations):
            optimized_content, readability_changes = self._optimize_readability(
                optimized_content
            )
            changes.extend(readability_changes)
            
        # Оптимизация мета-тегов
        meta_title, meta_description, meta_changes = self._optimize_meta_tags(
            content, keywords
        )
    ...
    def _optimize_headings(
        ...
        for heading in headings_with_missing_keywords:
            ...
            if best_keyword:
                ...
                optimized_content = (
                    optimized_content[:start] +
                    new_heading +
                    optimized_content[end:]
                )
    ...
    def _optimize_keywords(
        ...
        if missing_keywords_in_first_paragraph:
            ...
            if original_paragraph.strip().endswith('.'):
                new_paragraph = original_paragraph.rstrip() + f" Это также относится к теме {best_keyword}."
            else:
                new_paragraph = original_paragraph.rstrip() + f". Это также относится к теме {best_keyword}."
        ...
        for keyword in keywords:
            ...
            if density < 0.5:
                ...
                if len(paragraphs) > 1:
                    ...
                    # Проверяем, заканчивается ли параграф точкой
                    if original_paragraph.strip().endswith('.'):
                        new_paragraph = original_paragraph.rstrip() + f" Важно учитывать {keyword} при этом."
                    else:
                        new_paragraph = original_paragraph.rstrip() + f". Важно учитывать {keyword} при этом."
    ...
    def _optimize_readability(
        ...
        for sentence in long_sentences[:3]:  # Оптимизируем только первые 3 длинных предложения
            ...
            for i in range(middle_index, min(len(words), middle_index + 5)):
                if words[i].lower() in ['и', 'или', 'но', 'однако', 'также', 'тем', 'этом', 'этой', 'этих']:
                    split_index = i
                    break
    ...
    def _optimize_meta_tags(
        ...
        changes.append({
            ...
            'original': first_paragraph[:100] + "..." if len(first_paragraph) > 100 else first_paragraph,
    ...
    def _generate_meta_description(
        ...
        if keywords and keywords[0].lower() not in meta_description.lower():
            ...
            if len(meta_description) + len(keywords[0]) + 30 > self.optimal_description_length:
                meta_description = meta_description[:self.optimal_description_length - len(keywords[0]) - 30] + "..."
    ...
    def _calculate_optimization_stats(
        ...
        original_keyword_count = sum(original_content.lower().count(keyword.lower()) for keyword in keywords)
        optimized_keyword_count = sum(optimized_content.lower().count(keyword.lower()) for keyword in keywords)
