...
class StatisticalAnalyzer:
    ...
    def analyze_content(
        self, content: str, keywords: Optional[List[str]] = None, **kwargs
    ) -> Dict[str, Any]:
        """
        ...
        if keywords:
            results["keyword_relevance"] = self.analyze_keyword_relevance(
                content=content, keywords=keywords, extracted_keywords=results["extracted_keywords"]
            )
    ...
    def extract_ngrams(self, content: str) -> Dict[str, List[Dict[str, Any]]]:
        ...
        filtered_words = [
            word for word in words if word not in self.stopwords and word not in string.punctuation
        ]
    ...
    def analyze_topic(self, content: str) -> Dict[str, Any]:
        ...
        for sentence in sentences:
            ...
            keywords_in_sentence = sum(1 for kw in top_keywords if kw["keyword"] in sentence_lower)
    ...
    def analyze_keyword_relevance(
        self, content: str, keywords: List[str], extracted_keywords: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        avg_relevance = (
            sum(kr["relevance_score"] for kr in keyword_relevance.values()) / len(keywords)
            if keywords
            else 0
        )
