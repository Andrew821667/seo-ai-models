...
class LLMTokenManager:
    ...
    def _create_default_token_usage(self) -> Dict:
        ...
        return {
            ...
            "daily_usage": {today: {provider.value: 0 for provider in LLMProvider}},
            "history": [],
        }
    ...
    def get_daily_token_usage(
        self, provider: Union[LLMProvider, str], date: Optional[str] = None
    ) -> int:
        """
    ...
    def record_token_usage(
        self, provider: Union[LLMProvider, str], tokens: int, operation: str = "analysis"
    ) -> bool:
        """
        ...
        self.token_usage["total_usage"][provider] = (
            self.token_usage["total_usage"].get(provider, 0) + tokens
        )
        ...
        self.token_usage["daily_usage"][today][provider] = (
            self.token_usage["daily_usage"][today].get(provider, 0) + tokens
        )

        # Добавление записи в историю
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self.token_usage["history"].append(
            {"timestamp": timestamp, "provider": provider, "tokens": tokens, "operation": operation}
        )
    ...
    def get_recommended_provider(self, tokens: int) -> Tuple[str, float]:
        ...
        if not available_providers:
            return (
                ...
                tokens * self.TOKEN_COST_RATIO.get(LLMProvider.LOCAL.value, 0.1),
            )
    ...
    def get_usage_statistics(self) -> Dict[str, Any]:
        ...
        statistics = {
            ...
            "utilization_percentage": {
                provider.value: (
                    (
                        self.token_usage["total_usage"].get(provider.value, 0)
                        / self.token_limits.get(self.tier, {}).get(provider.value, 1)
                    )
                    * 100
                    if self.token_limits.get(self.tier, {}).get(provider.value, 0) > 0
                    else 0
                )
                for provider in LLMProvider
            },
