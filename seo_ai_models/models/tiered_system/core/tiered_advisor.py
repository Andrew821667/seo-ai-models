...
class ResourceOptimizer:
    ...
    def __init__(self, tier: Union[str, TierPlan], user_id: str, **kwargs):
        """
    ...
    def optimize_request(self, request_params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "llm_model" in optimized_params:
            ...
            if model_ranking.get(requested_model, 0) > model_ranking.get(tier_model, 0):
                ...
                logger.info(
                    f"Model downgraded from {requested_model} to {tier_model} for tier {self.tier}"
                )
        ...
        if "analysis_depth" in optimized_params:
            ...
            if requested_depth > tier_depth:
                ...
                logger.info(
                    f"Analysis depth reduced from {requested_depth} to {tier_depth} for tier {self.tier}"
                )
...
class FeatureGating:
    ...
    def get_feature_tiers(self, feature_name: str) -> List[str]:
        ...
        """
        if feature_name not in self.FEATURE_AVAILABILITY:
            # Проверяем, существует ли функция в конфигурации
            if feature_name not in self.FEATURE_AVAILABILITY:
                import logging

                logger = logging.getLogger(__name__)
                logger.warning(f"Feature '{feature_name}' not found in FEATURE_AVAILABILITY")
                return []

            # Получаем доступные уровни для функции
            available_tiers = []
            feature_config = self.FEATURE_AVAILABILITY[feature_name]

            for tier, is_available in feature_config.items():
                if is_available:
                    # Добавляем информацию об уровне
                    tier_info = {"tier": tier, "available": True, "feature": feature_name}

                    # Добавляем дополнительную информацию об уровне
                    if hasattr(self, "TIER_METADATA"):
                        tier_metadata = self.TIER_METADATA.get(tier, {})
                        tier_info.update(
                            {
                                "display_name": tier_metadata.get("display_name", tier.title()),
                                "description": tier_metadata.get(
                                    "description", f"{tier.title()} tier access"
                                ),
                                "monthly_price": tier_metadata.get("monthly_price", 0),
                                "yearly_price": tier_metadata.get("yearly_price", 0),
                                "limits": tier_metadata.get("limits", {}),
                            }
                        )

                    available_tiers.append(tier_info)

            # Сортируем уровни по приоритету (если определен)
            if hasattr(self, "TIER_PRIORITY"):
                tier_priority = self.TIER_PRIORITY
                available_tiers.sort(key=lambda x: tier_priority.get(x["tier"], 999))
            ...
            logger = logging.getLogger(__name__)
            logger.debug(
                f"Available tiers for feature '{feature_name}': {[t['tier'] for t in available_tiers]}"
            )

            return available_tiers

        return [
            tier for tier, available in self.FEATURE_AVAILABILITY[feature_name].items() if available
        ]
...
class TieredAdvisor:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.resource_optimizer = (
            ResourceOptimizer(tier=self.tier, user_id=self.user_id, **kwargs)
    ...
    def analyze_content(self, content: str, **kwargs) -> Dict[str, Any]:
        ...
        if self.optimize_resources:
            self.resource_optimizer.update_usage_stats(
                tokens_used=len(content.split()), content_size=len(content)
            )
    ...
    def _perform_analysis(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.analysis_settings[
            "sentiment_analysis"
        ] and self.feature_gating.is_feature_available("advanced_analysis"):
            result["sentiment_analysis"] = self._analyze_sentiment(content)

        if self.analysis_settings["semantic_analysis"] and self.feature_gating.is_feature_available(
            "advanced_analysis"
        ):
        ...
        if self.analysis_settings[
            "competitor_analysis"
        ] and self.feature_gating.is_feature_available("competitor_analysis"):
        ...
        if self.analysis_settings["llm_optimization"] and self.feature_gating.is_feature_available(
            "llm_integration"
        ):
        ...
        if self.analysis_settings["custom_metrics"] and self.feature_gating.is_feature_available(
            "custom_metrics"
        ):
    ...
    def _calculate_readability(self, content: str) -> Dict[str, Any]:
        ...
        flesch_ease = max(
            ...
            min(
                ...
                - 84.6 * (sum(len(word) for word in words) / max(1, len(words)) / 100),
            ),
        )
        ...
        return {
            ...
            "readability_level": (
                "Easy" if flesch_ease > 70 else "Medium" if flesch_ease > 50 else "Difficult"
            ),
        }
    ...
    def _analyze_competitors(self, content: str, keyword: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendations": ["Add more content about topic1", "Cover topic2 in more detail"],
        }
    ...
    def _calculate_custom_metrics(
        self, content: str, custom_metrics: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        """
        # Простая заглушка для демонстрации
        return {"custom_metric_1": 0.75, "custom_metric_2": 0.85, "custom_metric_3": 0.65}
    ...
    def _generate_recommendations(self, analysis_results: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if "competitor_analysis" in analysis_results:
            competitor_recommendations = analysis_results["competitor_analysis"].get(
                "recommendations", []
            )
    ...
    def get_resource_usage(self) -> Dict[str, Any]:
        ...
        return {"tier": self.tier, "usage_stats": None, "tier_limits": None}
