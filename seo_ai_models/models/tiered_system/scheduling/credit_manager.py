...
class CreditManager:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.used_credits = {
            credit_type: 0 for credit_type in CreditType
        }
        ...
        self.logger.info(f"CreditManager инициализирован для пользователя {self.user_id} с планом {tier.value}")
    ...
    def use_credits(
        ...
        for credit_type, cost in operation_costs.items():
            if self.credit_limits.get(credit_type, 0) != -1:  # Не считаем для безлимитных
                self.used_credits[credit_type] = self.used_credits.get(credit_type, 0) + (cost * count)
    ...
    def _check_reset_counters(self) -> None:
        ...
        if now >= self.reset_time:
            ...
            self.logger.info(f"Счетчики использования кредитов сброшены для пользователя {self.user_id}")
