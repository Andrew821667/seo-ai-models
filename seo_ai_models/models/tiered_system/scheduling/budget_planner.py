...
class BudgetPlanner:
    ...
    def _create_default_budget_plan(self) -> Dict:
        ...
        tier_allocation = self.budget_allocation.get(self.tier, self.budget_allocation[TierPlan.MICRO.value])
        ...
        daily_limits = {
            credit_type.value: (daily_limit_percentage / 100) * available_credits.get(credit_type.value, 0)
            for credit_type in CreditType
        }
    ...
    def get_allocation_for_operation(self, operation: str, credit_type: Union[CreditType, str] = None) -> Dict[str, float]:
        """
    ...
    def check_operation_availability(self, operation: str, required_credits: Dict[str, float]) -> bool:
        """
    ...
    def _get_current_operation_usage(self, operation: str, credit_type: str) -> float:
        ...
        if current_date in self.budget_plan["usage_history"] and \
           operation in self.budget_plan["usage_history"][current_date] and \
           credit_type in self.budget_plan["usage_history"][current_date][operation]:
    ...
    def forecast_needs(self, days: int = 30, usage_pattern: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
        """
        ...
        last_7_days = [(datetime.now() - timedelta(days=i)).strftime('%Y-%m-%d') for i in range(1, 8)]
        ...
        forecast = {
            ...
            "projected_usage": {
                credit_type: amount * days 
                for credit_type, amount in avg_daily_usage.items()
            },
            "available_credits": self.budget_plan["available_credits"],
            "projected_balance": {
                credit_type: self.budget_plan["available_credits"].get(credit_type, 0) - (amount * days)
                for credit_type, amount in avg_daily_usage.items()
            }
        }
    ...
    def generate_optimization_suggestions(self) -> List[Dict[str, Any]]:
        ...
        for credit_type, balance in projected_balance.items():
            if balance < 0:
                days_to_zero = forecast["available_credits"].get(credit_type, 0) / forecast["avg_daily_usage"].get(credit_type, 1)
                
                suggestions.append({
                    "type": "credit_depletion",
                    "credit_type": credit_type,
                    "days_to_zero": round(days_to_zero),
                    "current_balance": self.budget_plan["available_credits"].get(credit_type, 0),
        ...
        for day, operations in usage_history.items():
            for operation, credits in operations.items():
                if operation not in operations_usage:
                    operations_usage[operation] = {
                        credit_type: 0.0 for credit_type in credits
                    }
    ...
    def get_budget_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "utilization_ratio": {
                credit_type: (total_usage.get(credit_type, 0.0) / self.budget_plan["available_credits"].get(credit_type, 1.0)) 
                              if self.budget_plan["available_credits"].get(credit_type, 0.0) > 0 else 0.0
