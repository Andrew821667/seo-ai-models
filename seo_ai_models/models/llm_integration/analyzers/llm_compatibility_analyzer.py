...
class LLMCompatibilityAnalyzer:
    ...
    def analyze_compatibility(
        self, content: str, llm_types: Optional[List[str]] = None, budget: Optional[float] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _generate_compatibility_prompt(self, content: str, llm_types: List[str]) -> str:
        ...
        final_prompt = (
            base_prompt + llm_specific_instructions + "\n\nТекст для анализа:\n" + content
        )

        return final_prompt

    def _process_compatibility_result(
        self, result: Dict[str, Any], llm_types: List[str]
    ) -> Dict[str, Any]:
        """
    ...
    def _analyze_large_content(
        self, content: str, llm_types: List[str], budget: Optional[float] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _combine_chunk_results(
        ...
        if not summary and averaged_scores:
            ...
            if overall_score > 7:
                ...
                summary = (
                    "Контент умеренно оптимизирован для LLM, требуются существенные улучшения."
                )
            else:
