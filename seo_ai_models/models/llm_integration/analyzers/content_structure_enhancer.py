...
class ContentStructureEnhancer:
    ...
    def analyze_structure(
        self, content: str, content_type: str = "article", budget: Optional[float] = None
    ) -> Dict[str, Any]:
        """
    ...
    def enhance_structure(
        ...
        analysis_result = self.analyze_structure(
            content, content_type, budget * 0.3 if budget else None
        )
        ...
        return self._process_structure_enhancement_result(
            result, content, content_type, provider, model
        )
    ...
    def _generate_structure_enhancement_prompt(
        self,
        content: str,
        content_type: str,
    ...
    def _process_structure_analysis_result(
                ...
                for element in [
                    ...
                    if element in section_title.lower():
                        ...
                        if element_key != "overall":
                            structure_data["improvements"][element_key] = re.findall(
                                r"[-*]\s*(.*?)(?:\n|$)", section_content
                            )
    ...
    def _process_structure_enhancement_result(
        self,
        result: Dict[str, Any],
    ...
    def _clean_enhanced_content(self, content: str) -> str:
        ...
        clean_content = re.sub(
            r"^(Вот улучшенная версия|Улучшенная структура|Вот реструктурированный).*?\n\n",
            "",
            content,
            flags=re.IGNORECASE,
        )
        clean_content = re.sub(
            r"\n\n(Я сохранил|Я внес следующие изменения|Теперь контент).*?$",
            "",
            clean_content,
            flags=re.IGNORECASE,
        )
    ...
    def _analyze_structural_changes(self, original: str, enhanced: str) -> Dict[str, Any]:
        ...
        paragraphs_changes = {
            ...
            "avg_original_length": sum(len(p) for p in original_paragraphs)
            / max(1, len(original_paragraphs)),
            "avg_enhanced_length": sum(len(p) for p in enhanced_paragraphs)
            / max(1, len(enhanced_paragraphs)),
        }
        ...
        lists_changes = {
            ...
            "difference": (enhanced_lists + enhanced_numbered_lists)
            - (original_lists + original_numbered_lists),
    ...
    def _analyze_large_content_structure(
        self, content: str, content_type: str, budget: Optional[float] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _combine_structure_chunk_results(
    ...
    def generate_structural_template(
        self,
        topic: str,
        content_type: str = "article",
    ...
    def _clean_template(self, template: str) -> str:
        ...
        clean_template = re.sub(
            r"^(Вот структурный шаблон|Шаблон для|Структурный шаблон).*?\n\n",
            "",
            template,
            flags=re.IGNORECASE,
        )
        clean_template = re.sub(
            r"\n\n(Этот шаблон|Данный шаблон|Шаблон оптимизирован).*?$",
            "",
            clean_template,
            flags=re.IGNORECASE,
        )
