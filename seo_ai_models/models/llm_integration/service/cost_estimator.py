...
from ..common.utils import estimate_tokens, estimate_cost, chunk_text
...
class CostEstimator:
    ...
    def __init__(self):
        ...
        self.usage_stats = {"total_cost": 0.0, "total_tokens": 0, "providers": {}}
    ...
    def optimize_content_for_budget(
        self, content: str, provider: str, model: str, budget: float
    ...
    def get_usage_stats(
        ...
        if provider:
            ...
            if provider not in self.usage_stats["providers"]:
                return {"provider": provider, "total_cost": 0.0, "total_tokens": 0, "models": {}}
            ...
            if model:
                ...
                if days:
                    ...
                    period_input_tokens = sum(
                        stats["input_tokens"] for stats in filtered_daily_stats.values()
                    )
                    period_output_tokens = sum(
                        stats["output_tokens"] for stats in filtered_daily_stats.values()
                    )
                    period_total_tokens = sum(
                        stats["total_tokens"] for stats in filtered_daily_stats.values()
                    )
                    period_requests = sum(
                        stats["requests"] for stats in filtered_daily_stats.values()
                    )
    ...
    def forecast_monthly_cost(self, daily_budget: Optional[float] = None) -> Dict[str, Any]:
        ...
        if not seven_day_stats["providers"]:
            return {"monthly_forecast": 0.0, "daily_average": 0.0, "monthly_by_provider": {}}
        ...
        if daily_budget is not None:
            ...
            budget_info = {
                ...
                "days_until_budget_exceeded": (
                    (monthly_budget / monthly_forecast) * 30
    ...
    def optimize_usage(self, target_cost_reduction: float) -> Dict[str, Any]:
        ...
        if not stats["providers"]:
            return {"message": "Недостаточно данных для оптимизации", "recommendations": []}
                ...
                model_usage.append(
                    {
                        ...
                        "cost_per_token": model_stats["total_cost"]
                        / max(model_stats["total_tokens"], 1),
                    }
                )
                            ...
                            cheaper_alternatives.append(
                                {
                                    ...
                                    "estimated_monthly_savings": usage["cost"]
                                    * (savings_percent / 100),
                                }
                            )
                    ...
                    recommendations.append(
                        {
                            ...
                            "estimated_monthly_savings": best_alternative[
