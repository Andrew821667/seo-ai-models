...
from ..common.constants import LLM_PROVIDERS, LLM_MODELS, DEFAULT_MODELS, TOKEN_COSTS
from ..common.exceptions import (
    ProviderNotSupportedError,
    ModelNotSupportedError,
    BudgetExceededError,
)
from ..common.utils import estimate_cost, extract_scores_from_text, parse_json_response
...
class MultiModelAgent:
    ...
    def select_optimal_model(
        self, task_type: str, content_length: int, budget: Optional[float] = None
    ) -> Tuple[str, str]:
        """
        ...
        if budget is not None:
            ...
            if cost > budget:
                ...
                for available_provider in available_providers:
                    ...
                    provider_cost = estimate_cost(
                        input_tokens, output_tokens, available_provider, provider_model
                    )
    ...
    def generate_parallel(
        self, prompt: str, providers: List[Tuple[str, str]], timeout: int = 60
    ) -> List[Dict[str, Any]]:
        """
        ...
        def execute_request(provider_tuple):
            ...
            optimized_prompt = self.prompt_generator.optimize_for_provider(prompt, provider, model)
            # Генерируем ответ
            try:
                ...
                return {"provider": provider, "model": model, "error": str(e), "text": None}
    ...
    def combine_results(
        self, results: List[Dict[str, Any]], combination_method: str = "weighted"
    ) -> Dict[str, Any]:
        """
        ...
        if combination_method == "best":
            ...
            for model, info in scores_by_model.items():
                info["normalized_weight"] = (
                    info["weight"] / total_weight if total_weight > 0 else 1 / len(scores_by_model)
                )
            ...
            return {
                ...
                "models": [r.get("model") for r in valid_results],
                "weights": {
                    model: info["normalized_weight"] for model, info in scores_by_model.items()
                },
            }
    ...
    def extract_insights(
        ...
        try:
            ...
            if not insights_data or "insights" not in insights_data:
                insights_data = {"insights": [], "summary": "", "key_topics": []}
                ...
                for line in result["text"].split("\n"):
                    if ":" in line and any(c.isdigit() for c in line):
                        ...
                        insights_data["insights"].append(
                            {"insight": insight_text, "importance": importance}
                        )
