...
from ..common.constants import LLM_PROVIDERS, LLM_MODELS, DEFAULT_REQUEST_PARAMS
...
class AnthropicProvider(LLMProvider):
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "claude-3-sonnet"):
        ...
        if not self.api_key:
            raise ValueError(
                "API ключ Anthropic не найден. Укажите его явно или через переменную окружения ANTHROPIC_API_KEY"
            )
...
class GigaChatProvider(LLMProvider):
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "gigachat-pro"):
        ...
        if not self.api_key:
            raise ValueError(
                "API ключ GigaChat не найден. Укажите его явно или через переменную окружения GIGACHAT_API_KEY"
            )
        ...
        try:
            ...
            if "access_token" in result:
                ...
                raise APIResponseError(
                    "gigachat", response.status_code, "В ответе отсутствует токен доступа"
                )
...
class LLMService:
    ...
    def estimate_cost(
        self, prompt: str, provider: str, expected_output_length: int = None
    ) -> float:
        """
