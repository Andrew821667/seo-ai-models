...
class LocalLLMManager:
    ...
    def _scan_available_models(self) -> None:
        ...
        for model_dir in os.listdir(self.models_dir):
            ...
            if (
                "config.json" in model_files
                or "model.bin" in model_files
                or "pytorch_model.bin" in model_files
            ):
    ...
    def select_optimal_model(
        self, prompt: str, required_quality: str = "medium", max_cost: Optional[float] = None
    ) -> str:
        """
        ...
        if required_quality == "low":
            ...
            candidates = ["TinyLlama-1.1B-Chat", "phi-2", "gemma-2b", "qwen1.5-0.5b"]
            ...
            candidates = ["Llama-2-13b-chat", "mixtral-8x7b", "qwen1.5-14b", "qwen1.5-32b"]
        ...
        if not available_candidates:
            if self.available_models:
                self.logger.warning(
                    f"Нет доступных моделей для качества {required_quality}, выбираем из доступных"
                )
        ...
        if max_cost is not None:
            ...
            if affordable_models:
                ...
                self.logger.warning(
                    f"Нет моделей, удовлетворяющих ограничению по стоимости {max_cost} руб."
                )
