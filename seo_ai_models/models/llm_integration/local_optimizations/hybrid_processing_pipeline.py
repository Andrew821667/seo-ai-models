...
class HybridProcessingPipeline:
    ...
    def _strategy_cloud_first(
        ...
        if budget is not None and cloud_cost > budget:
            self.logger.warning(
                f"Стоимость облачной обработки ({cloud_cost} руб.) превышает бюджет ({budget} руб.), используем локальную модель"
            )
        ...
        try:
            ...
            if selected_local_model is None:
                selected_local_model = self.local_llm_manager.select_optimal_model(
                    prompt=prompt, required_quality=required_quality, max_cost=budget
                )
            ...
            if quality_score < self.quality_threshold and self.fallback_mode:
                self.logger.info(
                    f"Качество локального ответа ({quality_score:.2f}) ниже порога ({self.quality_threshold}), используем облачную модель"
                )
                ...
                if budget is not None and cloud_cost > budget:
                    self.logger.warning(
                        f"Стоимость облачной обработки ({cloud_cost} руб.) превышает бюджет ({budget} руб.), используем локальный результат"
                    )
            ...
            if self.fallback_mode:
                ...
                if budget is not None and cloud_cost > budget:
                    self.logger.warning(
                        f"Стоимость облачной обработки ({cloud_cost} руб.) превышает бюджет ({budget} руб.), возвращаем ошибку"
                    )
    ...
    def _strategy_cost_optimized(
        ...
        local_cost = (
            self.local_llm_manager.local_cost_per_1k_tokens.get(selected_local_model, 0.0005)
            * (len(prompt.split()) + max_tokens)
            / 1000
        )
        ...
        if budget is not None and cloud_cost > budget:
            ...
            self.logger.info(
                f"Облачная обработка ({cloud_cost} руб.) превышает бюджет ({budget} руб.), используем локальную модель"
            )
            ...
            self.logger.info(
                f"Локальная обработка ({local_cost:.6f} руб.) значительно дешевле облачной ({cloud_cost} руб.), используем локальную модель"
            )
            ...
            if required_quality == "high":
                self.logger.info(
                    f"Для высокого качества используем облачную модель (стоимость: {cloud_cost} руб.)"
                )

                result = self._use_cloud_model(prompt, max_tokens, temperature, budget, cloud_model)
            else:
                self.logger.info(
                    f"Используем локальную модель для оптимизации затрат (стоимость: {local_cost:.6f} руб.)"
                )
        ...
        result["cost_comparison"] = {
            ...
            "savings": (
                cloud_cost - local_cost if result["processing_info"]["method"] == "local" else 0
            ),
        }
    ...
    def _strategy_quality_optimized(
        ...
        if budget is not None and cloud_cost > budget:
            ...
            self.logger.info(
                f"Для оптимизации качества используем облачную модель (стоимость: {cloud_cost} руб.)"
            )
    ...
    def _strategy_balanced(
        ...
        if budget is not None and cloud_cost > budget:
            ...
            self.logger.info(
                f"Для сложного запроса или высокого качества используем облачную модель (стоимость: {cloud_cost} руб.)"
            )
            ...
            self.logger.info(
                "Для простого запроса и невысокого качества используем локальную модель"
            )
            ...
            if self.cost_optimization_level == "high":
                self.logger.info(
                    "Согласно высокому уровню оптимизации затрат используем локальную модель"
                )
            ...
            # Если уровень оптимизации затрат низкий, предпочитаем облачную модель
                ...
                self.logger.info(
                    "Согласно низкому уровню оптимизации затрат используем облачную модель"
                )
            ...
            # Для среднего уровня оптимизации выбираем по соотношению стоимость/качество
                ...
                if cloud_cost < 5.0:  # Пример порога стоимости
                    self.logger.info(
                        f"Облачная обработка недорогая ({cloud_cost} руб.), используем облачную модель"
                    )
    ...
    def _use_local_model(
        ...
        try:
            ...
            if quality_score < self.quality_threshold and self.fallback_mode:
                ...
                # Запрос к облачной модели
                cloud_result = self.llm_service.query(
                    prompt=prompt, max_tokens=max_tokens, temperature=temperature, budget=budget
                )
    ...
    def _estimate_cloud_cost(
        self, prompt: str, max_tokens: int, model: Optional[str] = None
    ) -> float:
        """
    ...
    def configure(
        ...
        cost_optimization_level: Optional[str] = None,
    ) -> None:
        """
        ...
        if cost_optimization_level is not None:
            if cost_optimization_level not in ["low", "medium", "high"]:
                self.logger.warning(
                    f"Уровень оптимизации затрат {cost_optimization_level} не поддерживается, используем 'medium'"
                )
