...
from ..common.utils import parse_json_response, chunk_text
...
class SemanticStructureExtractor:
    ...
    def extract_semantic_structure(
        ...
        if method == "statistical" and not HAS_SKLEARN:
            ...
            if self.multi_model_agent is None:
                raise RuntimeError(
                    "Ни один из методов не доступен. Установите scikit-learn или предоставьте LLMService."
                )
            method = "llm"
        ...
        if method == "llm":
            ...
            llm_result = self._extract_structure_llm(
                content, n_topics, budget * 0.7 if budget else None
            )
    ...
    def _extract_structure_llm(
        self, content: str, n_topics: int, budget: Optional[float] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _extract_structure_statistical(self, content: str, n_topics: int) -> Dict[str, Any]:
        ...
        if not HAS_SKLEARN:
            raise ImportError(
                "scikit-learn не установлен. Невозможно использовать статистический метод."
            )
        ...
        for topic_idx in range(n_topics):
            topic_paragraphs = [
                paragraphs[i] for i in range(len(paragraphs)) if paragraph_topics[i] == topic_idx
            ]
        ...
        for i, keywords in enumerate(topic_keywords):
            semantic_clusters.append(
                {"name": f"Кластер {i+1}", "keywords": keywords, "related_clusters": []}
            )
    ...
    def _process_structure_extraction_result(
        ...
        if not structure_data or "topics" not in structure_data:
            ...
            for i, topic_title in enumerate(topic_sections):
                ...
                if len(topic_sections) <= 2:
                    ...
                    end_idx = (
                        (i + 1) * chunk_size if i < len(topic_sections) - 1 else len(paragraphs)
                    )
            ...
            if not structure_data["topics"]:
                structure_data["topics"].append(
                    {"name": "Основная тема", "keywords": [], "paragraphs": paragraphs}
                )

            # Извлекаем общие ключевые слова
            keyword_section = re.search(
                r"(?:Общие ключевые слова|Keywords|Ключевые слова).*?:(.*?)(?:\n\n|\n[А-Я]|$)",
                text,
                re.DOTALL,
            )
            ...
            cluster_sections = re.findall(
                r"(?:Семантический кластер|Cluster)\s+\d+:?\s+(.+?)(?:\n|$)", text
            )

            for cluster_title in cluster_sections:
                cluster = {"name": cluster_title.strip(), "keywords": [], "related_clusters": []}

                # Ищем ключевые слова для кластера
                keyword_section = re.search(
                    r"(?:Ключевые слова|Keywords).*?:(.*?)(?:\n\n|\n[А-Я]|$)",
                    text[text.find(cluster_title) :],
                    re.DOTALL,
                )
            ...
            if summary_section:
                ...
                structure_data["content_summary"] = (
    ...
    def _extract_large_content_structure(
        self, content: str, method: str, n_topics: int, budget: Optional[float] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _combine_extraction_chunk_results(
        ...
        for i, topic in enumerate(all_topics):
            ...
            for j, other_topic in enumerate(all_topics):
                ...
                keyword_similarity = self._keyword_similarity(
                    topic.get("keywords", []), other_topic.get("keywords", [])
                )
        ...
        for i, cluster in enumerate(all_clusters):
            ...
            for j, other_cluster in enumerate(all_clusters):
                ...
                keyword_similarity = self._keyword_similarity(
                    cluster.get("keywords", []), other_cluster.get("keywords", [])
                )
                ...
                for target_cluster in combined_clusters:
                    if (
                        self._text_similarity(
                            related_cluster.get("name", ""), target_cluster["name"]
                        )
                        > 0.5
                    ):
                        related.append(
    ...
    def _combine_extraction_results(
        self, llm_result: Dict[str, Any], stat_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        for llm_topic in llm_topics:
            ...
            for stat_topic in stat_topics:
                ...
                keyword_similarity = self._keyword_similarity(
                    llm_topic.get("keywords", []), stat_topic.get("keywords", [])
                )
                ...
                if name_similarity > 0.5 or keyword_similarity > 0.3:
                    ...
                    matched_topic = {
                        ...
                        "keywords": list(
                            set(llm_topic.get("keywords", []) + stat_topic.get("keywords", []))
                        ),
                        "paragraphs": list(
                            set(llm_topic.get("paragraphs", []) + stat_topic.get("paragraphs", []))
                        ),
                    }
        ...
        for stat_topic in stat_topics:
            ...
            for llm_topic in llm_topics:
                ...
                keyword_similarity = self._keyword_similarity(
                    stat_topic.get("keywords", []), llm_topic.get("keywords", [])
                )
        ...
        for llm_cluster in llm_clusters:
            ...
            for stat_cluster in stat_clusters:
                ...
                keyword_similarity = self._keyword_similarity(
                    llm_cluster.get("keywords", []), stat_cluster.get("keywords", [])
                )
                ...
                if name_similarity > 0.5 or keyword_similarity > 0.3:
                    ...
                    matched_cluster = {
                        ...
                        "keywords": list(
                            set(llm_cluster.get("keywords", []) + stat_cluster.get("keywords", []))
                        ),
        ...
        for stat_cluster in stat_clusters:
            ...
            for llm_cluster in llm_clusters:
                ...
                keyword_similarity = self._keyword_similarity(
                    stat_cluster.get("keywords", []), llm_cluster.get("keywords", [])
                )
    ...
    def _merge_topics(self, topics: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        """
        if not topics:
            return {"name": "", "keywords": [], "paragraphs": []}
        ...
        return {"name": name, "keywords": unique_keywords, "paragraphs": unique_paragraphs}
    ...
    def _merge_clusters(self, clusters: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        """
        if not clusters:
            return {"name": "", "keywords": [], "related_clusters": []}
        ...
        return {"name": name, "keywords": unique_keywords, "related_clusters": unique_related}
