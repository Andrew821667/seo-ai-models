...
class FeatureImportanceAnalyzer:
    ...
    def analyze_feature_importance(self, factors: Optional[List[str]] = None, 
    ...
    def compare_feature_importance(self, llm_types: List[str] = ["generic", "chat", "search"],
        ...
        for factor in factors:
            ...
            importances = [
                comparison["importance_by_type"][llm_type].get(factor, 0)
                for llm_type in llm_types
            ]
    ...
    def analyze_content_factors(self, content: str, 
                              factors: Optional[List[str]] = None,
                              llm_type: str = "generic",
                              budget: Optional[float] = None) -> Dict[str, Any]:
        """
        ...
        # Обрабатываем результат анализа
        return self._process_content_analysis_result(result, factors, content, llm_type, provider, model)
    ...
    def _generate_content_analysis_prompt(self, content: str, factors: List[str], 
                                       llm_type: str) -> str:
        """
    ...
    def _process_importance_result(self, result: Dict[str, Any], 
                                factors: List[str],
                                llm_type: str,
                                provider: str,
        ...
        if not importance_data or "factor_importance" not in importance_data:
            ...
            top_section = re.search(r"(?:топ|лучш|важн).*?факторы?.*?:(.*?)(?:\n\n|\n#|$)", text, re.IGNORECASE | re.DOTALL)
            if top_section:
                ...
                sorted_factors = sorted(
                    importance_data["factor_importance"].items(), 
                    key=lambda x: x[1], 
                    reverse=True
                )
            ...
            bottom_section = re.search(r"(?:наимен|наихуд|менее).*?факторы?.*?:(.*?)(?:\n\n|\n#|$)", text, re.IGNORECASE | re.DOTALL)
            if bottom_section:
            ...
            summary_section = re.search(r"(?:резюме|вывод|заключ).*?:(.*?)(?:\n\n|\n#|$)", text, re.IGNORECASE | re.DOTALL)
            if summary_section:
    ...
    def _process_content_analysis_result(self, result: Dict[str, Any], 
                                      factors: List[str],
                                      content: str,
                                      llm_type: str,
                                      provider: str,
