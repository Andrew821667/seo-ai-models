...
class LLMDimensionMap:
    ...
    def train(
        ...
        if features.shape[1] != self.features_dim:
            raise ValueError(
                f"Размерность признаков ({features.shape[1]}) не соответствует ожидаемой ({self.features_dim})"
            )

        if len(feature_names) != self.features_dim:
            raise ValueError(
                f"Количество имен признаков ({len(feature_names)}) не соответствует размерности признаков ({self.features_dim})"
            )
