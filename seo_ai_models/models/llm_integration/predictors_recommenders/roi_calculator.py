...
class ROICalculator:
    ...
    def calculate_roi(
        ...
        if current_traffic is None:
            ...
            current_ranking = self.rank_predictor.predict_ranking(
                content=current_content, query=query, industry=industry
            )
            ...
            if "position_estimate" in current_ranking["ranking_prediction"]:
                position = current_ranking["ranking_prediction"]["position_estimate"][
                    "estimated_position"
                ]
        ...
        # Расчет ROI
        additional_revenue = (
            traffic_increase * conversion_rate * conversion_value * timeframe_months
        )
        roi = (
            (additional_revenue - implementation_costs) / implementation_costs
        ...
        payback_period = (
            implementation_costs / monthly_additional_revenue
    ...
    def _simulate_recommendation_implementation(
        self, content: str, recommendation: str, query: str
    ) -> Optional[str]:
        """
    ...
    def estimate_baseline_traffic(
        self, query: str, position: int, industry: Optional[str] = None
    ) -> int:
        """
        ...
        # Выполняем запрос к LLM
        try:
            response = self.llm_service.query(prompt=prompt, max_tokens=50, temperature=0.3)
    ...
    def calculate_detailed_roi(
        ...
        if business_data is None:
            business_data = {
                ...
                "llm_conversion_rate": self.default_conversion_rates.get(industry, 0.02)
                * 1.2,  # Обычно выше на 20%
                "traditional_conversion_value": self.default_conversion_values.get(industry, 15000),
                "llm_conversion_value": self.default_conversion_values.get(industry, 15000)
                * 1.1,  # Обычно выше на 10%
            }
        ...
        for i, recommendation in enumerate(recommendations):
            ...
            traditional_increase = (
                business_data.get("traditional_traffic", 0) * traditional_increase_rate
            )
        ...
        # Общий ROI
        roi = (
            (total_additional_revenue - implementation_costs) / implementation_costs
            if implementation_costs > 0
            else 0
        )
        roi_percent = roi * 100
        ...
        result = {
            ...
            "traditional": {
                ...
                "new_total_traffic": business_data.get("traditional_traffic", 0)
                + traditional_traffic_increase,
            ...
            "total": {
                "current_traffic": business_data.get("traditional_traffic", 0)
                + business_data.get("llm_traffic", 0),
                "traffic_increase": traditional_traffic_increase + llm_traffic_increase,
                "new_total_traffic": (
                    business_data.get("traditional_traffic", 0)
                    + business_data.get("llm_traffic", 0)
                    + traditional_traffic_increase
                    + llm_traffic_increase
                ),
