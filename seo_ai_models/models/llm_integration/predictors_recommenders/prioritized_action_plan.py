...
class PrioritizedActionPlan:
    ...
    def create_action_plan(
        ...
        implementation_plan = self._generate_implementation_plan(plan_phases, resources_level)
    ...
    def _generate_recommendations(
        self, content: str, query: str, industry: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        response = self.llm_service.query(prompt=prompt, max_tokens=2000, temperature=0.5)
        ...
        recommendations = []
        tokens_per_rec = (
            response.get("tokens", {}).get("total", 0) / len(recommendations_data)
            if recommendations_data
            else 0
        )
        cost_per_rec = (
            response.get("cost", 0) / len(recommendations_data) if recommendations_data else 0
        )
    ...
    def _analyze_and_group_recommendations(
        self, recommendations: List[Dict[str, Any]], roi_details: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for rec in recommendations:
            ...
            prioritization_score = (priority * impact / max(1, difficulty)) * (
                1 + math.log(1 + traffic_increase)
            )

            # Определяем группу на основе категории рекомендации
            category = rec.get("category", "content")
            group = self._determine_recommendation_group(
                category, rec.get("type", "traditional_seo")
            )
    ...
    def _create_plan_phases(
        self, analyzed_recommendations: List[Dict[str, Any]], resources_level: str
    ) -> Dict[str, Any]:
        """
        ...
        for rec in analyzed_recommendations:
            ...
            if (
                difficulty <= 2
                and priority >= 4
                and len(plan_phases["immediate"]["tasks"]) < immediate_tasks_max
            ):
                phase = "immediate"
            elif (
                difficulty <= 3
                and priority >= 3
                and len(plan_phases["short_term"]["tasks"]) < short_term_tasks_max
            ):
                phase = "short_term"
            elif (
                difficulty <= 4
                and priority >= 3
                and len(plan_phases["mid_term"]["tasks"]) < mid_term_tasks_max
            ):
                phase = "mid_term"
    ...
    def _generate_implementation_plan(
        self, plan_phases: Dict[str, Any], resources_level: str
    ) -> Dict[str, Any]:
        """
    ...
    def _generate_plan_summary(
        self, plan_phases: Dict[str, Any], roi_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        summary = {
            ...
            "description": self._generate_summary_description(
                total_tasks,
                traditional_seo_tasks,
                llm_optimization_tasks,
                total_costs,
                roi_data.get("roi_percent", 0),
                roi_data.get("payback_period_months", 0),
            ),
        }
    ...
    def _generate_summary_description(
        self,
        total_tasks: int,
        traditional_seo_tasks: int,
        llm_optimization_tasks: int,
        total_costs: float,
        roi_percent: float,
        ...
        description = (
            f"План включает {total_tasks} задач: {traditional_seo_tasks} по традиционному SEO "
        )
    ...
    def generate_resource_allocation(
        self, action_plan: Dict[str, Any], available_resources: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        resource_allocation = {"resource_types": resource_types, "phases": {}}

        # Распределяем ресурсы по фазам и задачам
        for phase_key, phase_data in (
            action_plan.get("implementation_plan", {}).get("phases", {}).items()
        ):
