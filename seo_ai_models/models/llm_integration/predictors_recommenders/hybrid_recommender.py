...
class HybridRecommender:
    ...
    def generate_recommendations(
        ...
        llm_recommendations = self._get_llm_recommendations(content, query, industry)
        ...
        result = {
            ...
            "tokens": {
                "traditional": sum(
                    rec.get("tokens", {}).get("total", 0) for rec in traditional_recommendations
                ),
                "llm": sum(rec.get("tokens", {}).get("total", 0) for rec in llm_recommendations),
            },
            "cost": (
                sum(rec.get("cost", 0) for rec in traditional_recommendations)
    ...
    def _get_traditional_recommendations(
        ...
        if self.enhanced_seo_advisor:
            ...
            seo_analysis = self.enhanced_seo_advisor.analyze_content(
                content=content, target_keywords=[query], industry=industry
            )
            ...
            traditional_recommendations.append(
                {
                    ...
                    "cost": (
                        response.get("cost", 0) / len(recommendations_data)
    ...
    def _analyze_recommendation_conflicts(
        self,
        traditional_recommendations: List[Dict[str, Any]],
    ...
    def _generate_conflict_resolution(
        self, trad_recommendation: str, llm_recommendation: str
    ) -> str:
        """
        ...
        resolution += (
            "сфокусировавшись на качестве контента, его структурировании и информативности, "
        )
        resolution += "что будет полезно для обоих типов поисковых систем."

        return resolution

    def _combine_and_prioritize_recommendations(
        self,
        traditional_recommendations: List[Dict[str, Any]],
        llm_recommendations: List[Dict[str, Any]],
        ...
        for rec in traditional_recommendations:
            ...
            weighted_priority = (
                rec.get("priority", 0) * rec.get("impact", 0) * balance_weights["traditional_seo"]
            )

            # Добавляем рекомендацию с взвешенным приоритетом
            all_recommendations.append({**rec, "weighted_priority": weighted_priority})

        # Добавляем рекомендации для LLM-поисковиков
        for rec in llm_recommendations:
            # Рассчитываем взвешенный приоритет
            weighted_priority = (
                rec.get("priority", 0) * rec.get("impact", 0) * balance_weights["llm_optimization"]
            )
            ...
            conflict_resolutions.append(
                {
                    ...
                    "weighted_priority": 5
                    * 5
                    * (balance_weights["traditional_seo"] + balance_weights["llm_optimization"])
                    / 2,
                    "tokens": {"total": 0},
                    "cost": 0,
                }
    ...
    def _generate_recommendation_summary(self, recommendations: List[Dict[str, Any]]) -> str:
        ...
        traditional_count = sum(
            1 for rec in recommendations if rec.get("type") == "traditional_seo"
        )
        ...
        if high_priority_recs:
            ...
            summary += ", ".join(
                [rec.get("recommendation", "")[:50] + "..." for rec in high_priority_recs[:3]]
            )

        return summary

    def simulate_recommendation_impact(
        self, content: str, query: str, recommendation: str, industry: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        # Выполняем запрос к LLM
        response = self.llm_service.query(prompt=prompt, max_tokens=2000, temperature=0.7)
        ...
        if not simulation_data:
            ...
            content_match = re.search(
                r"Модифицированный контент:(.*?)(?:Описание изменений:|$)",
                simulation_text,
                re.DOTALL,
            )
            changes_match = re.search(
                r"Описание (внесенных )?изменений:(.*?)$", simulation_text, re.DOTALL
            )

            modified_content = content_match.group(1).strip() if content_match else content
            changes_description = (
                changes_match.group(2).strip() if changes_match else "Нет описания изменений."
            )
        ...
        result = {
            ...
            "tokens": {
                ...
                "total": (
                    response.get("tokens", {}).get("total", 0)
