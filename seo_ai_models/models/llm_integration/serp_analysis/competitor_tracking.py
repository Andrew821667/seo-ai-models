...
class CompetitorTracking:
    ...
    def _save_competitor_data(
        ...
        if (
            query
            not in self.competitors_data[competitor_id]["content_hashes"][content_hash]["queries"]
        ):
            self.competitors_data[competitor_id]["content_hashes"][content_hash]["queries"][
                query
            ] = []
    ...
    def _rank_competitors(
        self, competitors_results: List[Dict[str, Any]], our_result: Optional[Dict[str, Any]] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _analyze_competitor_strategies(
        self, competitors_results: List[Dict[str, Any]], our_result: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        strategies_analysis = {
            ...
            "average_metrics": {
                ...
                "citation_rate": (
                    sum(result.get("citation_rate", 0) for result in all_results) / len(all_results)
                    if all_results
                    else 0
                ),
    ...
    def _generate_citation_insights(
        self, all_results: List[Dict[str, Any]], our_result: Optional[Dict[str, Any]] = None
    ) -> str:
        """
        ...
        if our_result:
            ...
            avg_visibility = sum(result.get("visibility_score", 0) for result in all_results) / len(
                all_results
            )
    ...
    def _generate_recommendations(
        ...
        if not our_result:
            recommendations.append(
                f"Проанализируйте контент лидера - {top_competitor.get('name', 'топ-конкурента')} - для понимания стратегии оптимизации."
            )

            if content_length_correlation > 0.5:
                avg_length = sum(result.get("content_length", 0) for result in all_results) / len(
                    all_results
                )
                recommendations.append(
                    f"Оптимальная длина контента для данной тематики составляет примерно {avg_length:.0f} символов."
                )

            recommendations.append(
                "Отслеживайте изменения в контенте конкурентов для выявления стратегий оптимизации."
            )
        ...
        if our_citation_rate < top_citation_rate * 0.8:
            recommendations.append(
                f"Повысьте цитируемость контента, изучив стратегию лидера ({top_competitor.get('name', 'топ-конкурента')})."
            )

        if our_visibility_score < top_visibility_score * 0.8:
            recommendations.append(
                "Улучшите видимость контента в LLM-поисковиках, добавив больше уникальной и полезной информации."
            )
        ...
        if content_length_correlation > 0.5:
            if our_content_length < top_content_length * 0.8:
                recommendations.append(
                    f"Увеличьте длину контента примерно до {top_content_length} символов, добавив релевантную информацию."
                )
            elif our_content_length > top_content_length * 1.2:
                recommendations.append(
                    f"Сократите и структурируйте контент, сделав его более лаконичным (примерно до {top_content_length} символов)."
                )

        # Общие рекомендации
        recommendations.append(
            "Регулярно отслеживайте цитируемость контента конкурентов для выявления новых трендов и стратегий."
        )

        # Если рекомендаций мало, добавляем общие
        if len(recommendations) < 3:
            recommendations.append(
                "Улучшите структуру контента, добавив подзаголовки и маркированные списки для лучшего восприятия LLM."
            )
            recommendations.append(
                "Добавьте больше фактической информации, статистику и данные исследований для повышения авторитетности."
            )

        return recommendations

    def get_competitor_history(
        self, competitor_id: str, time_period: Optional[int] = None
    ) -> Dict[str, Any]:
        """
