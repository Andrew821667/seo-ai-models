...
def setup_services(openai_api_key: Optional[str] = None) -> Dict[str, Any]:
    ...
    if not api_key:
        logger.warning("API ключ OpenAI не найден. Укажите его явно или через переменную окружения OPENAI_API_KEY")
        return {}
...
def analyze_content(services: Dict[str, Any], content: str, analysis_type: str) -> None:
    ...
    try:
        ...
        if analysis_type == "compatibility":
            ...
            logger.info(f"Общая оценка структуры: {result.get('structure_scores', {}).get('overall', 0)}")
            ...
            sorted_factors = sorted(
                result.get("factor_importance", {}).items(),
                key=lambda x: x[1],
                reverse=True
            )
...
def enhance_content(services: Dict[str, Any], content: str, enhancement_type: str) -> None:
    ...
    try:
        ...
        if enhancement_type == "structure":
            ...
            logger.info(f"Заголовки: {changes.get('headings', {}).get('original_count', 0)} -> {changes.get('headings', {}).get('enhanced_count', 0)}")
            logger.info(f"Абзацы: {changes.get('paragraphs', {}).get('original_count', 0)} -> {changes.get('paragraphs', {}).get('enhanced_count', 0)}")
            logger.info(f"Списки: {changes.get('lists', {}).get('original_count', 0)} -> {changes.get('lists', {}).get('enhanced_count', 0)}")
            logger.info(f"Выделения: {changes.get('highlights', {}).get('original_bold', 0) + changes.get('highlights', {}).get('original_italic', 0)} -> {changes.get('highlights', {}).get('enhanced_bold', 0) + changes.get('highlights', {}).get('enhanced_italic', 0)}")
            ...
            logger.info(f"Заголовки: {structure.get('headings_count', 0)} (H1: {structure.get('h1_count', 0)}, H2: {structure.get('h2_count', 0)}, H3: {structure.get('h3_count', 0)})")
            logger.info(f"Секции: {structure.get('sections', 0)}")
            logger.info(f"Списки: {structure.get('bullet_lists_count', 0) + structure.get('numbered_lists_count', 0)}")
...
def main() -> None:
    ...
    analysis_types = ["compatibility", "citability", "structure", "eeat", "semantic", "feature_importance"]
