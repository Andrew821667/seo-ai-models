...
class TestSerpAnalysis(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.llm_service.query = MagicMock(
            return_value={
        ...
        self.citability_scorer.score_citability = MagicMock(
            return_value={
                "citability_score": 7,
    ...
    def test_llm_serp_analyzer(self, mock_citability_scorer1, mock_citability_scorer2):
        ...
        from seo_ai_models.models.llm_integration.serp_analysis.llm_serp_analyzer import (
            LLMSerpAnalyzer,
        )
        ...
        with patch.object(serp_analyzer, "_check_content_citation", return_value=True):
            with patch.object(serp_analyzer, "_evaluate_relevance", return_value=0.7):
                with patch.object(
                    serp_analyzer, "_determine_citation_position", return_value={"is_cited": True}
                ):
    ...
    def test_citation_analyzer(self, mock_citability_scorer1, mock_citability_scorer2):
        ...
        from seo_ai_models.models.llm_integration.serp_analysis.citation_analyzer import (
            CitationAnalyzer,
        )
        ...
        with patch(
            "seo_ai_models.models.llm_integration.serp_analysis.citation_analyzer.LLMSerpAnalyzer"
        ) as mock_serp_analyzer_class:
    ...
    def test_competitor_tracking(self, mock_citability_scorer1, mock_citability_scorer2):
        ...
        # Мокаем LLMSerpAnalyzer
        with patch(
            "seo_ai_models.models.llm_integration.serp_analysis.competitor_tracking.LLMSerpAnalyzer"
        ) as mock_serp_analyzer_class:
            mock_serp_analyzer = MagicMock()
            mock_serp_analyzer_class.return_value = mock_serp_analyzer
            ...
            with patch.object(
                competitor_tracking,
                "_analyze_competitor_strategies",
                return_value={"top_competitor": {}, "recommendations": []},
            ):
                        ...
                        competitors = [
                            {
                                "id": "comp1",
                                "name": "Competitor 1",
                                "content": "competitor 1 content",
                            },
                            {
                                "id": "comp2",
                                "name": "Competitor 2",
                                "content": "competitor 2 content",
                            },
                        ]
    ...
    def test_industry_benchmarker(self, mock_citability_scorer1, mock_citability_scorer2):
        ...
        # Мокаем LLMSerpAnalyzer
        with patch(
            "seo_ai_models.models.llm_integration.serp_analysis.industry_benchmarker.LLMSerpAnalyzer"
        ) as mock_serp_analyzer_class:
            mock_serp_analyzer = MagicMock()
            mock_serp_analyzer_class.return_value = mock_serp_analyzer
            ...
            with patch.object(industry_benchmarker, "_analyze_content_metrics", return_value={}):
                with patch.object(
                    industry_benchmarker, "_generate_benchmark_analysis", return_value={}
                ):
                    with patch.object(
                        industry_benchmarker, "_generate_benchmark_recommendations", return_value=[]
                    ):
