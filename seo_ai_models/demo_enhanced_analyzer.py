...
def analyze_url(url, keywords):
    ...
    parser = SPAParser(wait_for_load=7000, wait_for_timeout=45000, record_ajax=True)
    ...
    print(
        f" - Изображений: {metrics.get('images_count', 0) if 'images_count' in metrics else 'Н/Д'}"
    )
...
def main():
    ...
    parser.add_argument(
        "--keywords", help="Ключевые слова через запятую", default="seo,optimization,content"
    )
    ...
    if results:
        ...
        with open(filename, "w", encoding="utf-8") as f:
            json.dump(
                ...
                ensure_ascii=False,
                indent=2,
                default=lambda x: x if not isinstance(x, datetime) else x.isoformat(),
            )
