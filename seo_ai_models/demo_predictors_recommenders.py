...
from seo_ai_models.models.llm_integration.predictors_recommenders.llm_rank_predictor import (
    LLMRankPredictor,
)
from seo_ai_models.models.llm_integration.predictors_recommenders.hybrid_recommender import (
    HybridRecommender,
)
from seo_ai_models.models.llm_integration.predictors_recommenders.roi_calculator import (
    ROICalculator,
)
from seo_ai_models.models.llm_integration.predictors_recommenders.prioritized_action_plan import (
...
def setup_services(openai_api_key: Optional[str] = None) -> Dict[str, Any]:
    ...
    prioritized_action_plan = PrioritizedActionPlan(
        llm_service, prompt_generator, roi_calculator, hybrid_recommender
    )
...
def demonstrate_hybrid_recommender(services: Dict[str, Any]) -> None:
    ...
    logger.info(
        "HybridRecommender генерирует рекомендации, оптимизированные как для традиционных, так и для LLM-поисковиков."
    )
    logger.info("Основные методы:")
    ...
    logger.info(
        "3. [LLM Optimization] Добавьте конкретные примеры использования облачных вычислений"
    )
    logger.info("4. [LLM Optimization] Включите статистику экономии затрат при переходе на облако")
    logger.info(
        "5. [Hybrid] Создайте структурированные блоки 'вопрос-ответ' для повышения видимости в обоих типах поиска"
    )
...
def demonstrate_roi_calculator(services: Dict[str, Any]) -> None:
    ...
    logger.info(
        "ROICalculator рассчитывает потенциальный возврат инвестиций от внедрения рекомендаций."
    )
    logger.info("Основные методы:")
    logger.info("- calculate_roi: Рассчитывает ROI от внедрения рекомендаций")
    logger.info(
        "- calculate_detailed_roi: Рассчитывает детальный ROI с разделением на традиционные и LLM-поисковики"
    )
    logger.info("- estimate_baseline_traffic: Оценивает базовый трафик для запроса и позиции")

    logger.info("\nПример расчета ROI для рекомендаций:")
    logger.info(
        "1. Оценка текущего трафика: 700 визитов (500 из традиционных поисковиков, 200 из LLM)"
    )
...
def demonstrate_prioritized_action_plan(services: Dict[str, Any]) -> None:
    ...
    logger.info(
        "4. Долгосрочные действия (6-12 месяцев) - стратегические задачи с высоким долгосрочным ROI"
    )

    logger.info("\nПример приоритизации задач:")
    logger.info(
        "- Высокий приоритет: Добавление информации об авторе (+5 к E-E-A-T, сложность: 1, ROI: 1200%)"
    )
    logger.info(
        "- Средний приоритет: Добавление FAQ-секции (+4 к цитируемости, сложность: 2, ROI: 450%)"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Демонстрация улучшенных предикторов и рекомендаций"
    )
    parser.add_argument("--api_key", help="API ключ OpenAI")
    parser.add_argument(
        "--demo",
        choices=["all", "rank", "recommendations", "roi", "plan"],
        default="all",
